...
Citizen.CreateThread(function()
    exports["desirerp-keybinds"]:registerKeyMapping("", "Player", "Tackle", "+plyTackel", "-plyTackel")
...
Citizen.CreateThread(function()
    while true do
        ...
        TriggerServerEvent("desirerp-base:updatecoords", coords.x, coords.y, coords.z)
        Wait(13000)
    end
end)
			...
			if dstCheck < 3 then
				...
				if dstCheck < 1 then
					...
					if IsControlJustReleased(1, 38) then
						...
						DoScreenFadeOut(500)
						Wait(1000)
						TriggerEvent("desirerp-base:clearStates")
						exports["desirerp-base"]:getModule("SpawnManager"):Initialize()
						Wait(1000)
					end
				end
			end
    ...
    while true do
        ...
        if recentsearch == 0 then
            ...
            if endDistance < 0.7 or endDistance2 < 0.7 then
                ...
                currentWeapons = exports["desirerp-inventory"]:GetCurrentWeapons()
...
AddEventHandler("police:rob", function()
    ...
    if not exports["isPed"]:isPed("dead") then
        ...
        local finished = exports["desirerp-taskbar"]:taskBar(3500, "Robbing", false, true)
...
AddEventHandler('KneelHU', function()
    ...
    if (DoesEntityExist(player) and not IsEntityDead(player)) then
        ...
        local finished = exports["desirerp-taskbar"]:taskBar(2500, "Surrendering")
...
AddEventHandler('police:wshotgun', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
            ...
            local finished = exports["desirerp-taskbar"]:taskBar(2000, "Unracking Shotgun")
            if finished == 100 then
...
AddEventHandler('police:wrifle', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
            ...
            loadAnimDict('anim@narcotics@trash')
            TaskPlayAnim(PlayerPedId(), 'anim@narcotics@trash', 'drop_front', 0.9, -8, 1900, 49, 3.0, 0, 0, 0)
            local finished = exports["desirerp-taskbar"]:taskBar(2000, "Unracking Rifle")
            if finished == 100 then
...
AddEventHandler('police:woxy', function()
    ...
    if vehFront > 0 then
        ...
        local finished = exports["desirerp-taskbar"]:taskBar(4000, "Grabbing Scuba Gear")
        if finished == 100 then
            loadAnimDict('anim@narcotics@trash')
            TaskPlayAnim(PlayerPedId(), 'anim@narcotics@trash', 'drop_front', 0.9, -8, 1900, 49, 3.0, 0, 0, 0)
...
AddEventHandler('giveRadioToPed', function()
    ...
    if (not exports["desirerp-inventory"]:hasEnoughOfItem("radio", 1, false)) then
...
AddEventHandler('swatcopLoadout', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
            ...
            local finished = exports["desirerp-taskbar"]:taskBar(2500, "Changing to Swat")
            if finished == 100 then
                ChangeToSkinNoUpdatePolice("s_m_y_swat_01")
            end
            ClearPedTasks(PlayerPedId())
...
AddEventHandler('police:fingerPrintPlayer', function()
    local finished = exports["desirerp-taskbar"]:taskBar(8000,"DNA Testing")
    if finished == 100 then
        t, distance = GetClosestPlayer()
        if(distance ~= -1 and distance < 5) then
...
AddEventHandler('police:gsr', function(t)
    ...
    local finished = exports["desirerp-taskbar"]:taskBar(15000, "GSR Testing")
    if finished == 100 then
        t, distance = GetClosestPlayer()
        if (distance ~= -1 and distance < 7) then
...
-- local finished = 0
-- if not isDead then
-- finished = exports["desirerp-taskbarskill"]:taskBar(1200,7)
-- end
...
AddEventHandler('police:getArrested2', function(cuffer)
    ...
    if not isDead and cuffAttemptCount < 4 then
        ...
        finished = exports["desirerp-ui"]:taskBarSkill(cuffAttemptTbl[cuffAttemptCount][1],
...
AddEventHandler('police:jailing', function(args)
    ...
    TriggerServerEvent('desirerp-jail:pJailedLog', tonumber(args[2]), tonumber(args[1]))
end)
...
exports["desirerp-polyzone"]:AddBoxZone("evidence_zone", vector3(474.26, -1005.19, 26.27), 1, 3.0, {
    heading=0,
    --debugPoly=true,
    minZ=23.27,
    maxZ=27.27
...
AddEventHandler('desirerp-polyzone:enter', function(pZone)
    if pZone == "evidence_zone" then
        inEvidenceZone = true
        exports['desirerp-interface']:showInteraction('/evidence [LOCKER ID]')
    end
end)

RegisterNetEvent('desirerp-polyzone:exit')
AddEventHandler('desirerp-polyzone:exit', function(zoneName)
...
RegisterNetEvent('desirerp-police:tenThirteenA')
AddEventHandler('desirerp-police:tenThirteenA', function()
    TriggerEvent('police:tenThirteenA')
end)

RegisterNetEvent('desirerp-police:tenThirteenB')
AddEventHandler('desirerp-police:tenThirteenB', function()
    TriggerEvent('police:tenThirteenB')
end)

RegisterNetEvent("desirerp-police:tenForteenA")
AddEventHandler("desirerp-police:tenForteenA", function()
    TriggerEvent('police:tenForteenA')
end)

RegisterNetEvent("desirerp-police:tenForteenB")
AddEventHandler("desirerp-police:tenForteenB", function()
    local pos = GetEntityCoords(PlayerPedId(), true)
    TriggerEvent('police:tenForteenB')
end)
    ...
    if t ~= nil and t ~= -1 then
        if (distance ~= -1 and distance < 5) then
            ...
            if myjob ~= "ems" and myjob ~= "doctor" then
                local bandages = exports["desirerp-inventory"]:getQuantity("bandage")
                if bandages == 0 then
                    return
                else
...
AddEventHandler("ems:stomachpump", function()
    ...
    if t ~= nil and t ~= -1 then
        if (distance ~= -1 and distance < 5) then
            local finished = exports["desirerp-taskbar"]:taskBar(10000, "Inserting stomach pump 🤢", false, true)
...
RegisterCommand('911', function(source, args)
    TriggerEvent('desirerp-dispatch:911')
...
AddEventHandler('FlipVehicle', function()
    ...
    local finished = exports["desirerp-taskbar"]:taskBar(18000, "Flipping Vehicle Over", false, true)
...
AddEventHandler("police:dorifto", function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if Driver == PlayerPedId() and IsVehicleModel(veh, defaultHash) or IsVehicleModel(veh, defaultHash2) or
            ...
            local finished = exports["desirerp-taskbar"]:taskBar(3000, "Toggling Drift Mode", false, true)
...
AddEventHandler('police:frisk', function()
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
        local items = exports["desirerp-inventory"]:getFullItemList()
        TriggerServerEvent("police:targetCheckWeapons", GetPlayerServerId(t),items)
        ExecuteCommand("e search")
        local finished = exports["desirerp-taskbar"]:taskBar(3000, "Frisking")
    else
        TriggerEvent("DoLongHudText", "No player near you!", 2)
    end
end)
...
RegisterInterfaceCallback('police:hireNewPersonel', function(data, cb)
    ...
    if dept == 1 then
        TriggerServerEvent('desirerp-police:hire_pd', data.values.cid, data.values.rank, data.values.callsign, 'police')
    elseif dept == 2 then
        TriggerServerEvent('desirerp-police:hire_pd', data.values.cid, data.values.rank, data.values.callsign, 'sheriff')
    elseif dept == 3 then
        TriggerServerEvent('desirerp-police:hire_pd', data.values.cid, data.values.rank, data.values.callsign, 'state')
    end
end)
...
AddEventHandler('police:hire', function()
    TriggerServerEvent('desirerp-police:sv:open_hire_menu')
end)

exports["desirerp-polytarget"]:AddCircleZone("desire_police_hire",  vector3(463.28, -984.29, 30.72), 0.55, {
    useZ = true
})

exports["desirerp-interact"]:AddPeekEntryByPolyTarget("desire_police_hire", {{
    event = "police:hire",
    id = "desire_police_hire",
    icon = "id-badge",
    label = "Hire Personel!",
    parameters = {},
}}, {
...
AddEventHandler('police:BillLog', function(pAmt, pPlayer)
    TriggerServerEvent('desirerp-jail:pBilledLog', pAmt, pPlayer)
end)
...
exports["desirerp-polyzone"]:AddBoxZone("desirerp_police_swat_armory", vector3(487.51, -997.09, 30.69), 1, 1.8, {
    heading=270,
    --debugPoly=true,
    minZ=27.09,
    maxZ=31.09
...
AddEventHandler('desirerp-polyzone:enter', function(name)
    if name == "desirerp_police_swat_armory" then
        if exports['isPed']:isPed('myjob') == "police" then
            pInSwatArmory = true
            exports["desirerp-interface"]:showInteraction("[E] Armory")
            SwatArmory()
        end
    end
end)

...
AddEventHandler('ChannelSet4', function()
  ...
  exports["desirerp-voice"]:setVoiceProperty("radioEnabled", true)
  exports['desirerp-voice']:addPlayerToRadio(4)
  PlaySound(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
...
AddEventHandler('ChannelSet5', function()
  ...
  exports["desirerp-voice"]:setVoiceProperty("radioEnabled", true)
  exports['desirerp-voice']:addPlayerToRadio(5)
  PlaySound(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
...
function openGui()
  ...
  elseif job == "ems" then
    Emergency = true
  elseif exports["desirerp-inventory"]:hasEnoughOfItem("scanner",1,false) then
    Emergency = true
  end
...
local function handleConnectionEvent(pChannel)
  ...
  if newChannel < 1.0 then
    ...
    pRadioChannel = true
    exports["desirerp-voice"]:setVoiceProperty("radioEnabled", true)
    exports['desirerp-voice']:addPlayerToRadio(newChannel)
  end
end
...
function hasRadio()
    if exports["desirerp-inventory"]:hasEnoughOfItem("radio", 1, false, true) or exports["desirerp-inventory"]:hasEnoughOfItem("civradio", 1, false, true)then
      return true
    else
      return false
    end
...
RegisterNUICallback('volumeUp', function(data, cb)
  ...
  TriggerEvent('desirerp-voice:SetRadioVolume', radioVolume)
...
RegisterNetEvent('desirerp-inventory:itemCheck')
AddEventHandler('desirerp-inventory:itemCheck', function (item, state, quantity)
    if item ~= "civradio" and item ~= "radio" and item ~="scanner" then return end
    if state or quantity > 0 then return end
    exports["desirerp-voice"]:removePlayerFromRadio(false)
    exports["desirerp-ui"]:sendAppEvent("radio", { value = 0, state = false })

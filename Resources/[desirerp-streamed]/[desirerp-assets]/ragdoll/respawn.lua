...
AddEventHandler("desirerp-menu:var:inServerFarm", function(pBool)
...
AddEventHandler('trycpr', function()
    ...
    if curDist < 5 or curDist2 < 5 then
        ...
        while penis < 10 do
            penis = penis + 1
            local finished = exports["desirerp-ui"]:taskBarSkill(math.random(2000, 6000), math.random(5, 10))
...
function releaseBody(dropItems)
    ...
    if dropItems then
        ...
        --TriggerServerEvent("desirerp-ragdoll:removeallitems")
    end
    ...
    local doctors = RPC.execute('desirerp-jobmanager:jobCount', 'doctor')
    ...
    TriggerServerEvent('desirerp-healthcare:playerRespawn', hosp)
...
AddEventHandler("desirerp:admin:currentDevmode", function(pDevmode)
    devmode = pDevmode
end)

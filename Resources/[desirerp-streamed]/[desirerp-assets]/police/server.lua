...
AddEventHandler('police:targetCheckBank', function(target)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(target)
...
AddEventHandler('police:jailGranted', function(args)
    ...
    local pdunit = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    if not pdunit:getVar("job") == "police" then
        local steamid, name = pdunit:getVar("name"), pdunit:getVar("steamid")

        exports["desirerp-log"]:AddLog("Exploiter", pdunit, "User Attempted to jail player while not on pd", {
            target = playerName,
            reason = reason,
            cid = cid,
...
AddEventHandler("police:deletecrimes", function(target)
    ...
    if not pdunit:getVar("job" == "police") then
        ...
        exports["desirerp-log"]:AddLog("Exploiter", pdunit, "User Attempted delete crimes while not on duty", {
            target = target,
            src = src
        })
        DropPlayer(src, "")
...
AddEventHandler('police:dnaAsk', function(t)
	local src = source
	local user2 = exports["desirerp-base"]:getModule("Player"):GetUser(t)
...
AddEventHandler("police:targetCheckInventory", function(target, status)
    local src = source
    local user2 = exports["desirerp-base"]:getModule("Player"):GetUser(target)
...
AddEventHandler('Gimmeurfuknmoney', function(target)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local targetply = exports["desirerp-base"]:getModule("Player"):GetUser(target)
...
AddEventHandler('police:SeizeCash', function(target)
    ...
    local userz = exports["desirerp-base"]:getModule("Player"):GetUser(target)
...
RegisterCommand('bill', function(source, args)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(tonumber(args[1]))
    local character = user:getCurrentCharacter()
    local user2 = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local character2 = user2:getCurrentCharacter()
    exports.ghmattimysql:execute('SELECT job FROM jobs_whitelist WHERE cid = ?', {character2.id}, function(result)
        if result[1].job == 'police' then
            ...
            TriggerServerEvent('desirerp-financials:business_money', args[2], 'police', 'add')
        end
    end)
end)
    ...
    if jobType == "police" or jobType == "ems" or jobType == "doctor" or jobType == "therapist" or jobType == "doc" then
        exports.ghmattimysql:execute("SELECT rank from jobs_whitelist WHERE job = @job AND cid = @cid", {
            ...
            if (result[1]) then
                ...
                exports["desirerp-log"]:AddLog("HC Added Job", "Character: " .. char.id .. " Changed Char: " .. arg ..
    ...
    if jobType == "police" or jobType == "ems" or jobType == "doctor" or jobType == "therapist" or jobType == "doc" then
        exports.ghmattimysql:execute("SELECT rank from jobs_whitelist WHERE job = @job AND cid = @cid", {
            ...
            if (result[1]) then
                ...
                exports["desirerp-log"]:AddLog("HC Removed Job", "Character: " .. char.id .. " Changed Char: " .. arg ..
...
-- 	local commands = exports["desirerp-base"]:getModule("Commands")
...
function jailUser(player, reason, cid, time, src)
    local jailer = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    if not jailer then
        return false
    end
    ...
    if not jailer:getVar("job") == "police" and not jailer:getVar("job") == "doc" then
        ...
        exports["desirerp-log"]:AddLog("Exploiter", jailer,
    ...
    TriggerEvent("desirerp-police:playerJailed", character)
...
AddEventHandler("911", function(args, pStreetName)
    ...
    local connect = {
        {
            ["color"] = color,
            ["title"] = "** DesireRP [Police] | 911 Call **",
            ["description"] = "Steam Name:"..pName.. " \n Steam ID: "..identifiers[1].. " \n 911 | " .. char.first_name .. " " .. char.last_name .. " \n Message: " .. tostring(message),
        }
    }
    PerformHttpRequest("https://discord.com/api/webhooks/1032832521181876224/qwUkbO4gy9CFXifTCx46PDTJCCnecZdV3hk_uupsxcCGuuydbV3kFQk4KO_byAKEqTWg", function(err, text, headers) end, 'POST', json.encode({username = "DesireRP | 911 Call", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
    ...
    local users = exports["desirerp-base"]:getModule("Player"):GetUsers()

    local emergencyPlayers = {}

    for k, v in pairs(users) do
        local user = exports["desirerp-base"]:getModule("Player"):GetUser(v)
        local job = user:getVar("job")
...
AddEventHandler("311", function(args)
    ...
    TriggerClientEvent("chatMessage", src,
        ...
        local connect = {
            {
                ["color"] = color,
                ["title"] = "** DesireRP [Police] | 311 Call **",
                ["description"] = "Steam Name:"..pName.. " \n Steam ID: "..identifiers[1].. " \n 911 | " .. char.first_name .. " " .. char.last_name .. " \n Message: " .. tostring(message),
            }
        }
        PerformHttpRequest("https://discord.com/api/webhooks/1002609535506202694/KFSwo-26My9Is41bxEcALf_k32DwDUAqIcmtfpQO4lg0zbOaaefCP62lphiK-ZlC7J89", function(err, text, headers) end, 'POST', json.encode({username = "DesireRP | 311 Call", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
...
AddEventHandler("police:targetCheckWeapons", function(playerId,items)
    local src = source
    local char = exports["desirerp-base"]:getModule("Player"):GetUser(playerId)
...
-- 	end
-- 	local caller = tostring(char.first_name) .. " " .. tostring(char.last_name)
-- 	exports["desirerp-log"]:AddLog("911 Chat", user, caller .. ": " .. message, {})
...
RegisterCommand('pstate', function(source, args)
    local src = source
    local users = exports["desirerp-base"]:getModule("Player"):GetUser(src)
...
RegisterServerEvent('desirerp-police:hire_pd')
AddEventHandler('desirerp-police:hire_pd', function(pCID, pRank, pCallsign, pDept)
...
RegisterServerEvent('desirerp-police:sv:open_hire_menu')
AddEventHandler('desirerp-police:sv:open_hire_menu', function()
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()

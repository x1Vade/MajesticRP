...
function preventVehicleExit()
    Citizen.CreateThread(function()
        ...
        disableControl = true
        local finished = exports["desirerp-taskbar"]:taskBar(options[math.random(1,3)],"Taking off Harness",true)
        if finished == 100 then
            TriggerEvent('desirerp-hud:harness_values', 0)
...
function toggleHarness()
    ...
    local finished = 0
    if harness then
        finished = exports["desirerp-taskbar"]:taskBar(5000,"Taking off Harness",true)
        TriggerEvent('desirerp-hud:harness_values', 0)
        TriggerEvent('seatbelt', false)
    else
        finished = exports["desirerp-taskbar"]:taskBar(5000,"Putting on Harness",true)
        TriggerEvent('desirerp-hud:harness_values', harnessDurability)

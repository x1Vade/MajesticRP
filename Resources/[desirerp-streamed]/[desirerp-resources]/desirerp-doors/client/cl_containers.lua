    ...
    if exports['desirerp-inventory']:hasEnoughOfItem('key1', 1) then
        if distance < 2 then
            if not pDoorUnlocked then
                pDoorUnlocked = true
                TriggerServerEvent('desirerp-doors:change-lock-state', 752, false) 
                TriggerServerEvent('desirerp-doors:change-lock-state', 753, false) 
            else
                pDoorUnlocked = false
                TriggerServerEvent('desirerp-doors:change-lock-state', 752, true) 
                TriggerServerEvent('desirerp-doors:change-lock-state', 753, true) 
            end
        end
    end
end)

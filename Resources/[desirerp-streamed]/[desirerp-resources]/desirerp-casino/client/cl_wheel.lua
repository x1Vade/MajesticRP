...
RegisterNetEvent("desirerp-luckywheel:doRoll")
AddEventHandler("desirerp-luckywheel:doRoll", function(_priceIndex) 
    _isRolling = true
	SetEntityHeading(_wheel, 328.00)
    Citizen.CreateThread(function()
        local speedIntCnt = 1
...
RegisterNetEvent("desirerp-luckywheel:rollFinished")
AddEventHandler("desirerp-luckywheel:rollFinished", function() 
    _isRolling = false
end)
...
function doRoll()
    local pChips = RPC.execute('desirerp-casino:getChips')
    ...
    if exports['desirerp-inventory']:hasEnoughOfItem('casinomember', 1) then
        if not _isRolling then
            ...
            if pChips >= 500 then
                TriggerEvent('desirerp-casino:removeChipsCL', 500)
...
RegisterNetEvent("desirerp-luckywheel:spinit")
AddEventHandler("desirerp-luckywheel:spinit", function() 
    if not _isRolling  then
        doRoll()
...
RegisterNetEvent("attempt:change:spin", function()
    local isEmployed = exports["desirerp-business"]:IsEmployedAt("casino")
	if isEmployed then 

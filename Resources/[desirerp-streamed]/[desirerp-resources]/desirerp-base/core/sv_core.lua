...
function DPX.Core.ConsoleLog(self, msg, mod, ply)
	...
	if ply and tonumber(ply) then
		TriggerClientEvent("desirerp-base:consoleLog", ply, msg, mod)
	end
end

AddEventHandler("onResourceStart", function(resource)
	TriggerClientEvent("desirerp-base:waitForExports", -1)
...
RegisterNetEvent("desirerp-base:playerSessionStarted")
...
AddEventHandler("desirerp-base:characterLoaded", function(user, char)
	local src = source
...
-- 		print("^1[desirerp-base] Paychecks Sent^0")
-- 	end
-- end)
	...
	exports.oxmysql:execute('SELECT `paycheck` FROM characters WHERE `id`= ?', {cid}, function(data)
		...
		if amount >= 1 then
			...
			local connect = {
				{
				  ["color"] = color,
				  ["title"] = "** DesireRP [Paychecks] **",
				  ["description"] = "State ID: "..cid.." \n First Name: "..pChar.first_name.." Last Name: "..pChar.last_name.." \n Amount: $"..amount,
				}
			  }
			  PerformHttpRequest("https://discord.com/api/webhooks/1033197618991484958/PsKn758Xkq2kObZX0kr5qbsyybIEudCpZHLg-T6t4pKQlbQPjfSG9A62sisPaFRKzbvz", function(err, text, headers) end, 'POST', json.encode({username = "DesireRP | Paycheck", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })

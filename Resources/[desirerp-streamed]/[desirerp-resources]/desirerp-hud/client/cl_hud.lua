...
RegisterNetEvent("desirerp-hud:EnableHud", function(status)
    isLoggedIn = status
    if status == true then
        exports['desirerp-interface']:showInteraction('[F] To pay respects')
        HudActive = false
    end
end)

RegisterNetEvent("desirerp-hud:changeRange", function(pRange)
    voice = pRange or 2
end)
...
RegisterNetEvent('desirerp-weapons:client:setWeaponFireRate', function (pAmount)
...
Citizen.CreateThread(function ()
	...
	while true do
        ...
        local pRadioActive = exports['desirerp-voice']:pRadioActive()
    ...
    while true do 
        ...
        if isLoggedIn then
            ...
            SendNUIMessage({
                ...
                gps = gps,
                gpu = gpu,
                nos = exports['desirerp-vehicles']:nitroLevel(),
                dev = developer,
                debug = debug,
...
RegisterNetEvent('desirerp-hud:pursuitModeHUD')
AddEventHandler('desirerp-hud:pursuitModeHUD', function(value)
...
RegisterNetEvent("desirerp-admin:currentDevmode")
AddEventHandler("desirerp-admin:currentDevmode", function(devmode)
    isBlocked = devmode
end)
--- CARHUD ---
...
RegisterNetEvent('desirerp-hud:ChangeThirst', function(amount)
...
RegisterNetEvent('desirerp-hud:ChangeHunger', function(amount)
	if isLoggedIn == true then
...
RegisterNetEvent("client:newStress", function(status, amount)
    ...
    if status == true then
        ...
        Wait(1000)
        TriggerServerEvent('desirerp-hud:UpdateStress_SV', pStress)
...
RegisterNetEvent('desirerp:admin:maxstats', function()
    ...
    TriggerServerEvent("desirerp-death:reviveSV", source)
...
RegisterNUICallback("time", function(data)
    exports['desirerp-carhud']:SetTimeEnabled(data.time)
end)

RegisterNUICallback("SaveSettings", function(data)
    local pInfo = exports["desirerp-base"]:getModule("LocalPlayer")
    ...
    exports['desirerp-carhud']:SetCompassFPS(tonumber(data.compassFPSValue))
    exports['desirerp-carhud']:SetSpeedFPS(tonumber(data.speedFPSValue))
    TriggerEvent('setModel', model)
    TriggerEvent('desirerp-phone:grabBackground', bg)
    TriggerEvent('changeBG', bg)
    TriggerEvent("desirerp-phone:setSomeShit",bg,model,twatNotify,callNotify)
    local circleTaskbar = GetResourceKvpInt(cid..'-circleTaskbar')
    if circleTaskbar == 1 then
        TriggerEvent("desirerp-preferences:setPreferences", true)
    elseif circleTaskbar == 0 then
        TriggerEvent("desirerp-preferences:setPreferences", false)
    end
...
AddEventHandler('desirerp-preferences:setPreferences', function(bool)
    local data = {
        ["hud.misc.circle.taskbar.enabled"] = bool
    }
    exports["desirerp-interface"]:sendAppEvent('preferences', data)
end)

RegisterNetEvent("desirerp-hud:SetValues", function()
    local pInfo = exports["desirerp-base"]:getModule("LocalPlayer")
    local cid = exports['isPed']:isPed('cid')
    ...
    local model = exports['desirerp-phone']:getModelForHud()
    local background = exports['desirerp-phone']:getBackgroundForHud()
    local twat = exports['desirerp-phone']:getTwatForHud()
    local call = exports['desirerp-phone']:getCallForHud()
    ...
    exports['desirerp-carhud']:SetCompassFPS(SetCompassFPSValue)
    exports['desirerp-carhud']:SetSpeedFPS(SetSpeedFPSValue)
    ...
    if SetTimeToggled == 1 then
        exports['desirerp-carhud']:SetTimeEnabled(true)
    elseif SetTimeToggled == 0 then
        exports['desirerp-carhud']:SetTimeEnabled(false)
    end
...
RegisterCommand('ui-r', function()
    ...
    TriggerEvent('desirerp-banking:ResetUI')
    TriggerEvent('desirerp-radio:ResetUI')
    TriggerEvent('desirerp-menu:ResetUI')
    TriggerEvent('desirerp-interact:ResetUI')
    TriggerEvent('desirerp-mdt:ResetUI')
    TriggerEvent('phone:reset')
    TriggerEvent('clothing:close')
    exports['desirerp-interface']:showInteraction("Restarting UI .")
    Wait(1000)
    exports['desirerp-interface']:showInteraction("Restarting UI ..")
    Wait(1000)
    exports['desirerp-interface']:showInteraction("Restarting UI ...")
    Wait(1000)
    exports['desirerp-interface']:showInteraction("UI Successfully Restarted")
    Wait(1000)
    exports['desirerp-interface']:hideInteraction()
...
RegisterNetEvent('desirerp-hud:boosting_value')
AddEventHandler('desirerp-hud:boosting_value', function(pValue)

...
RPC.register('desirerp-fuel:PurchaseSuccessful', function(src, gprice, type)
	local pSrc = src
	...
	elseif type == "cash" then
		if user:getCash() >= gprice then
			user:removeMoney(gprice)
			TriggerClientEvent('desirerp-fuel:confirmCashPayment', pSrc)
			return true
		else
			TriggerClientEvent('DoLongHudText', pSrc, 'Not enough cash!', 2)
			return false
		end
	elseif type == "selfserve" then
		TriggerClientEvent('desirerp-fuel:confirmCashPayment', pSrc)
	end
end)

...
RegisterNUICallback('racing:event:start', function(data)
  local canJoinOrErr = exports["desirerp-racing"]:canJoinOrStartRace()
  if canJoinOrErr ~= true then
    exports["desirerp-phone"]:phoneNotification("racen",canJoinOrErr,"RACING", 3000)
    return
  end
  print(data.id, json.encode(data))
    exports["desirerp-racing"]:createPendingRace(data.id, data)
    Wait(100)
    SendNUIMessage({
...
RegisterNUICallback("racing:event:join", function(data)
  ...
  exports["desirerp-racing"]:joinRace(id, alias, cid)
  Wait(100)
  SendNUIMessage({
    openSection = "racing:event:update",
    canJoin = false,
...
RegisterNetEvent("desirerp-racing:removedActiveRaced")
AddEventHandler("desirerp-racing:removedActiveRaced", function(id)
  -- print("PUTANG INA")
    openRacingApp()
...
function openRacingApp()
  ...
  local alias = RPC.execute("GetAlias")
  print(alias)
  local _races = exports["desirerp-racing"]:getAllRaces()
...
AddEventHandler("desirerp-racing:api:updatedState", function()
  ...
  SendNUIMessage({
      ...
      raceCreated = _newRaces,
      scid = exports["isPed"]:isPed("cid"),
      canMakeMap = (exports['desirerp-inventory']:hasEnoughOfItem('racingusb2'))
  })
end)
...
RegisterNUICallback('racing:event:start', function()
  exports["desirerp-racing"]:startRace()
end)

RegisterNUICallback("racing:event:end", function()
  exports["desirerp-racing"]:endRace()
end)

RegisterNUICallback("racing:event:leave", function()
  exports["desirerp-racing"]:leaveRace()
end)


RegisterNUICallback("racing:map:save", function(data)
  if creating then
      TriggerEvent("desirerp-racing:cmd:racecreatedone")
      ...
      if veh ~= 0 and GetPedInVehicleSeat(veh, -1) == ped then
          ...
          TriggerEvent("desirerp-racing:cmd:racecreate", options)
          closeGui()
...
RegisterNUICallback('racing:map:cancel', function()
  creating = false
  TriggerEvent('desirerp-racing:cmd:racecreatecancel')
  openRacingApp()
end)
...
RegisterNUICallback('desirerp-phone:racing:setGPS', function(data)
  local mapID = data.mID
  exports["desirerp-racing"]:locateRace(mapID)
  phoneNotification("racen","GPS Set to your map.","RACING")
end)

RegisterNUICallback('desirerp-phone:racing:seePreview', function(data)
  local mapID = data.mID
  if not mapID then return end
  id = tostring(mapID)
  exports["desirerp-racing"]:previewRace(mapID)
...
RegisterNUICallback('desirerp-phone:r-users', function(data, cb)
  ...
  if race ~= nil then
    ...
    local racer = RPC.execute('desirerp-phone:racing:getRacer', raceID)
    cb(racer)
  end
  print(json.encode(newPendingRace[raceID]))
end)

AddEventHandler("desirerp-racing:api:startingRace", function(startTime)
  TriggerEvent("DoLongHudText", "Starting race in " .. tostring(startTime / 1000) .. " seconds")
end)

AddEventHandler("desirerp-racing:api:playerJoinedYourRace", function(characterId, name)
  TriggerEvent("chatMessage", "", {255, 0, 0}, "^1" .. name .. " joined your race")
end)

AddEventHandler("desirerp-racing:api:playerLeftYourRace", function(characterId, name)
  TriggerEvent("chatMessage", "", {255, 0, 0}, "^1" .. name .. " left your race")
end)

RegisterNetEvent('desirerp-racing:pendingRacenotif')
AddEventHandler("desirerp-racing:pendingRacenotif", function(race)
  ...
  exports["desirerp-phone"]:phoneNotification("racen","Pending race available...","From the PM", 12000)
end)
-- Citizen.CreateThread(function()
--   while true do
--     Wait(5000)

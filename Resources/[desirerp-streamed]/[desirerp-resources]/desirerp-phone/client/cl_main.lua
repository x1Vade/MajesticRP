...
local passWifi = 0
local selfie_Discord = "https://discord.com/api/webhooks/1032833578356183040/ShhNE_-APOBXdRhVCsNH_QagmuCRLmsoqEmZXwqcWVWv0S0hWCBJ28dOlrcCwiAon1Ms"
local wifipass1 = ""
...
RegisterNetEvent('desirerp-base:playerSpawned')
AddEventHandler('desirerp-base:playerSpawned', function()
  Wait(1000)
...
RegisterCommand('renten', function()
  TriggerEvent('desirerp-housing:rent_activate_cl')
end)

RegisterNetEvent('desirerp-housing:rent_activate_cl')
AddEventHandler('desirerp-housing:rent_activate_cl', function()
...
AddEventHandler('desirerp-phone:sanitationIn', function()
  -- print("SANITATION SIGN IN")
  sanitation = true
end)

RegisterNetEvent("desirerp-phone:grabBackground")
AddEventHandler("desirerp-phone:grabBackground", function(link)
    wallPaper = link
end)
...
RegisterNUICallback('wallpaper', function(data, cb)
  ...
  TriggerEvent('desirerp-phone:grabBackground', wallPaperSelecionado)
...
AddEventHandler('phone:reset', function(cidsent)
    ...
    wallPaper = ""
    Wait(1)
    TriggerServerEvent('desirerp-phone:grabWallpaper')
end)
...
RegisterNUICallback('submitSettings', function(data, cb)
  ...
  Wait(5)
  if bg ~= "" then
    TriggerEvent('desirerp-phone:grabBackground', bg)
...
RegisterNetEvent("desirerp-phone:setSomeShit", function(bg, model, twatNotify, callNotify)
  SetResourceKvp(exports['isPed']:isPed('cid').."-bg", bg)
  SetResourceKvpInt(exports['isPed']:isPed('cid').."-pmodel", model)
  SetResourceKvpInt(exports['isPed']:isPed('cid').."-ntwat", twatNotify)
  SetResourceKvpInt(exports['isPed']:isPed('cid').."-ncall", callNotify)
...
RegisterCommand("closephonegui", function()
  SendNUIMessage({
    openSection = "forceclose"
  })
end)
...
RegisterNUICallback('btnHousing', function()
  ...
  allNewHouses = {}
  myAccessHouse = {}
  --local apt = RPC.execute("desirerp-phone:apt")
  local housing = exports['desirerp-housing']:housingForPhone()
  local apartment = exports['desirerp-apartments']:getMyCurrentApartment()
  myHousess = RPC.execute("desirerp-phone:getCurrentOwned")
  accessHouse = RPC.execute('desirerp-phone:getAccessHouse_2')
  ...
  for i,v in pairs(myHousess) do
    ...
    table.insert(newHouses, {
      ...
      cid = v.cid,
      status = v.status,
      cat = exports['desirerp-housing']:getHousingCatFromPropertID(v.hid),
    })
  end
  for l,s in pairs(accessHouse) do
      table.insert(myAccessHouse, {
        ...
        cid = s.cid,
        status = s.status,
        cat = exports['desirerp-housing']:getHousingCatFromPropertID(s.house),
      })
  end
  SendNUIMessage({
    openSection = "housing",
...
RegisterNUICallback('btnKeyHolders', function(data, cb)
  local myKittens = {}
  print(data.hid)
  local keys = RPC.execute('desirerp-housing:getKeyHolderz', data.hid)
  SendNUIMessage({
...
RegisterNUICallback('whatthefuckdoesthisshitwork', function(data,cb)
  local cid = data.cid
  local hid = data.hid
  local success,msg = RPC.execute('desirerp-housing:removeKey:phone',cid, hid)
  if success then
...
RegisterNUICallback('GetCameras', function(data,cb)
  local pResult = RPC.execute("desirerp-phone:getCameras", exports['isPed']:isPed("cid"))
  cb({cameras = pResult})
end)

RegisterNUICallback('OpenCamera', function(data,cb)
  RPC.execute('desirerp-phone:openCamera', data.cameraId)
end)
...
--   while true do
--     Wait(3000)
--     local myHouse = RPC.execute("desirerp-phone:getCurrentOwned")
...
RegisterInterfaceCallback("desirerp-phone:signInHandler", function(data, cb)
  if data.key.sign_in then
    RPC.execute("desirerp-phone:addDOJ", data.key.job)
  else
    RPC.execute("desirerp-phone:removeDOJ", data.key.job)
  end
...
RegisterNUICallback('getActiveDOJ', function(data,cb)
      SendNUIMessage({
        ...
        penisinmybuttholefuckshit = RPC.execute("desirerp-phone:getDOJ")
      })
end)
...
RegisterNUICallback('abdulSignIn', function(data,cb)
    ...
    if taxiStatus then
      RPC.execute("desirerp-phone:addAbdulTaxi")
      addAbdulTaxi()
    ...
    elseif not taxiStatus then
      RPC.execute("desirerp-phone:removeAbdulTaxi")
      removeAbdulTaxi()
      local availableDrivers = getAbdulTaxi()
      SendNUIMessage({
        openSection = "addTaxi",
...
RegisterNetEvent("desirerp-phone:manualUpdateAbdulTaxies", function(availableDrivers)
  addAbdulTaxi()
  local availableDrivers = getAbdulTaxi()
  SendNUIMessage({
    openSection = "addTaxi",
...
function addAbdulTaxi () 
  currenttable = {}
  local table = RPC.execute("desirerp-phone:getAbdulTaxies")
--  local currenttable = {}
...
RegisterNUICallback('sellVeh', function(data)
  ...
  TriggerServerEvent('desirerp-phone:sendVehBill', data.paypal, data.plate, data.amount)
end)

RegisterNetEvent('desirerp-phone:sellCar')
AddEventHandler('desirerp-phone:sellCar', function(plate,amount,sId)
  sellCarPlate = plate
...
RegisterNUICallback('purchaseCar', function(data)
  TriggerServerEvent('desirerp-phone:purchasedCar', sellCarPlate, data.amount,data.sID)
end)

RegisterNUICallback('purchase_gas', function(data)
  TriggerEvent('desirerp-fuel:OfferAccepted')
end)

RegisterNetEvent('desirerp-hud:send_fuel_bill', function(pCost)
...
function SpawnVehicle(vehicle, x,y,z, Fuel, customized, plate, IsViewing, enginehealth, bodyhealth)
  if DoesEntityExist(spawningVeh) then
	...
	Citizen.CreateThread(function()			
        ...
        exports['desirerp-vehicles']:SetVehicleIdentifier(spawningVeh, vid)
        ...
        if not IsViewing then    
            ...
            RPC.execute("desirerp-garages:states", "Out", plate, exports['desirerp-menu']:currentGarage(), pUpdatedFuel)
        end
    end)
end
...
RegisterNUICallback('sethGps', function(data)
 local house = exports["desirerp-housing"]:retrieveHousingTableMapped()
...
RegisterNUICallback('unlockHouses', function(data)
  ...
  if statuus == "Unlock" then
    --TriggerEvent('desirerp-phone:h-Lock', hId,"Lock")
  else
    --TriggerEvent('desirerp-phone:h-Lock', hId,"Unlock")
  end
  TriggerServerEvent('desirerp-phone:house:status',hId,statuus)
  Wait(200)
  TriggerEvent('updateUIHousing')
end)

RegisterNetEvent("desirerp-phone:h-Lock", function(hId, status)
  TriggerServerEvent("desirerp-phone:h-Lock:sv", hId, status)
...
RegisterNUICallback('editHousingOwn', function()
  -- if exports['desirerp-housing']:isNearProperty() then
    if exports["desirerp-housing"]:canEdit() then
      print("HELLO!")
      local isComplete, _propertyID, dist, zone = exports['desirerp-housing']:findClosetedPerson()
      TriggerEvent("desirerp-housing:editOptions",{action = "enter", propertyId = _propertyID})
    else
      SendNUIMessage({
...
RegisterNUICallback('sellOwnHouse', function(data)
  local price = RPC.execute('desirerp-phone:getHousePrice', data.hid)
end)

RegisterNUICallback('giveAccess', function(data)
  local complete = RPC.execute('desirerp-phone:GiveAccess', data.hid,data.cid)
...
RegisterNUICallback('houseAccess', function(data)
  ...
  local access = RPC.execute('desirerp-phone:getAccessHouse', data.hid)
  local data = {
      {
        title = "Close",
        action = "desirerp-housing:close_access",
        key = {},
    },
  }
  local player = {}
  for i,k in pairs(access) do
    data[#data + 1] = {
          ...
          key = {"lockdown"},
          -- action = "desirerp-ui:apartmentsContext"
          children = {
            {
                title = "Remove Key",
  ...
  exports["desirerp-contexts"]:showContext(data)
end)

AddEventHandler("desirerp-housing:close_access", function()
  exports["desirerp-contexts"]:hideContext()
end)

AddEventHandler('desirerp-ui:apartmentsContext', function(data)
  RPC.execute("desirerp-phone:removeKeys",data[2],data[1])
  exports["desirerp-ui"]:hideContext(data)
end)
...
function hasDecrypt2()
    if exports["desirerp-inventory"]:hasEnoughOfItem("vpnxj",1,false,true) then
      return true
    else
      return false
    end
...
function hasTrucker()
    if exports["desirerp-base"]:getModule("LocalPlayer"):getVar("job") == "trucker"  then
      return true
    else
      return false
    end
...
function hasDecrypt()
    if exports["desirerp-inventory"]:hasEnoughOfItem("decrypterenzo",1,false) or exports["desirerp-inventory"]:hasEnoughOfItem("decryptersess",1,false) or exports["desirerp-inventory"]:hasEnoughOfItem("decrypterfv2",1,false) and not exports["isPed"]:isPed("disabled") or exports["desirerp-inventory"]:hasEnoughOfItem(80,1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasDevice()
    if exports["desirerp-inventory"]:hasEnoughOfItem("mk2usbdevice",1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasPhone()
    if
      (
      (exports["desirerp-inventory"]:hasEnoughOfItem("mobilephone",1,false) or 
      exports["desirerp-inventory"]:hasEnoughOfItem("stoleniphone",1,false) or 
      exports["desirerp-inventory"]:hasEnoughOfItem("stolens8",1,false) or
      exports["desirerp-inventory"]:hasEnoughOfItem("stolennokia",1,false) or
      exports["desirerp-inventory"]:hasEnoughOfItem("stolenpixel3",1,false) or
      exports["desirerp-inventory"]:hasEnoughOfItem("assphone",1,false) or
      exports["desirerp-inventory"]:hasEnoughOfItem("boomerphone",1,false))
...
function hasRadio()
    if (exports["desirerp-inventory"]:hasEnoughOfItem("radio",1,false) or exports["desirerp-inventory"]:hasEnoughOfItem("pdradio",1,false)) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function openGui()
  ...
  if hasPhone() then
    ...
    local near = exports['desirerp-housing']:isNearProperty()
    local house = RPC.execute("desirerp-phone:getHouses")
...
function canCreateRace()
  local racingCreateUsbItem = exports["desirerp-inventory"]:GetInfoForFirstItemOfName("racingusb0")
...
function canOpenRaceApp()
  local racingUsbItem = exports["desirerp-inventory"]:GetInfoForFirstItemOfName("racingusb2")
...
function canJoinRaces()
  local hasRaceUsbAndAlias = exports["desirerp-racing"]:getHasRaceUsbAndAlias()
...
function closeGui()
  ...
  -- TriggerEvent('desirerp-phone:UpdateStatePhone')
  pPhoneOpen = false
  insideDelivers = false
end

-- RegisterNetEvent('desirerp-phone:UpdateStatePhone')
-- AddEventHandler('desirerp-phone:UpdateStatePhone', function()
--     Wait(5)
...
RegisterNUICallback('updateMyWallpaper', function(data, cb)
  wallPaper = ""
  Wait(5)
  TriggerEvent('desirerp-phone:grabBackground', data.name .."?auto=compress&cs=tinysrgb&h=350")
...
AddEventHandler('phone:casino_paid', function(message)
  if exports['desirerp-phone']:pOpen() == false then 
    SendNUIMessage({openSection = "emailnotify", pEHandle = 'Payment Received.', pEMessages = message})
    SendNUIMessage({openSection = "phonemedio", timeout = "5200", pOpen = exports['desirerp-phone']:pOpen()}) 
  end
...
RegisterNetEvent('desirerp-spawn:confirm_spawned')
AddEventHandler('desirerp-spawn:confirm_spawned', function()
  serverSpawn = true
end)
...
RegisterNUICallback('jobcenter-setgps', function(data)
  ...
  if data.item == "oxy" then
    SetNewWaypoint(-442.3712, 36.659885)
    TriggerEvent("DoLongHudText", "GPS Set!")  	
  elseif data.item == "chopshop" then	
    SetNewWaypoint(-219.36, -1368.19)	
    TriggerEvent("DoLongHudText", "GPS Set!")  	
  elseif data.item == "criminal" then	
    SetNewWaypoint(-1849.759, 27.52968)	
    TriggerEvent("DoLongHudText", "GPS Set!")
  end	
end)
...
RegisterCommand('house', function()
  local isComplete, propertyID, dist, zone = exports['desirerp-housing']:findClosetedPerson()
  print("HOUSING",isComplete, propertyID, dist, zone)
end)
RegisterNUICallback('checkHouse', function()
  
 -- if exports["desirerp-housing"]:canEdit() then
    TriggerEvent('desirerp-housing:phone:check')
 -- else
 --   SendNUIMessage({
 --     openSection = "noProperty"
 --   })
...
RegisterNUICallback('leaveEditMode', function()
  print("LEAVING EDIT")
  TriggerEvent('desirerp-housing:editOptions', { action = "exit" })
end)
...
RegisterNUICallback('openFurnitureUI', function()
  ...
  TriggerEvent('desirerp-housing:editOptions', { action = "furniture" })
end)

RegisterNUICallback('setBackdoorUI', function()
  print("SET BACKDOOR")
  TriggerEvent('desirerp-housing:editOptions', { action = "backdoor" })
end)

RegisterNUICallback('setWardrobeUI', function()
  print("SET WARDROBE")
  TriggerEvent('desirerp-housing:editOptions', { action = "wardrobe" })
end)

RegisterNUICallback('setCraftingUI', function()
  print("SET CRAFTING")
  TriggerEvent('desirerp-housing:editOptions', { action = "crafting" })
end)

RegisterNUICallback('setGarageUI', function()
  print("SET GARAGE")
  TriggerEvent('desirerp-housing:editOptions', { action = "garage" })
end)

RegisterNUICallback('setStashUI', function()
  print("SET STASH")
  TriggerEvent('desirerp-housing:editOptions', { action = "stash" })
end)

RegisterNUICallback('sellsHouse', function()
  print("EDITING")
  TriggerEvent('desirerp-housing:phone:check',"edit")
end)

AddEventHandler('desirerp-phone:sentHouse', function(hasPrice,id,street,price,category)
...
AddEventHandler('desirerp-phone:editingHouse', function(id,street,price)
  print("EDITING HERE")
  SendNUIMessage({
...
RegisterNUICallback('purchaseHouse', function(data)
  ...
  if rentEnabled then
      print("can purchase")
   --   TriggerEvent('desirerp-phone:sentHouse')
      local success, msg = RPC.execute("desirerp-housing:buy", data.hid, data.price, 0, exports['isPed']:isPed("fullname"))
      TriggerEvent("DoLongHudText",msg,1)
     return
  end
...
-- AddEventHandler('desirerp-phone:noProperty', function()
...
RegisterNUICallback('editHousePrice', function(data)
  TriggerServerEvent('desirerp-housing:editPrice', data.hid,data.price)
end)

AddEventHandler('desirerp-phone:EnableSellRent', function()
    exports['desirerp-ui']:openApplication('textbox', {
    callbackUrl = 'desirerp-phone:SellRentEnable',
...
RegisterInterfaceCallback("desirerp-phone:SellRentEnable", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  print('THIS IS ENABLED RENT/SALE FOR PLAYER', data[1].value,json.encode(data))
  TriggerServerEvent('desirerp-housing:rent_activate_sv', data[1].value)
...
RegisterNetEvent('desirerp-phone:signInInfoTowing')
AddEventHandler('desirerp-phone:signInInfoTowing', function()
  phoneNotification("email", "Grab the towtruck parked in the bay.", "Job Center")
end)

RegisterNetEvent('desirerp-phone:towing_offer')
AddEventHandler('desirerp-phone:towing_offer', function()
...
RegisterNUICallback('accept_tow_job', function(data)
  TriggerEvent('desirerp-jobs:towing_accept')
end)

-- 24/7 Delivery --

RegisterNetEvent('desirerp-phone:signInInfoDelivery')
AddEventHandler('desirerp-phone:signInInfoDelivery', function()
  phoneNotification("email", "Signed in as 24/7 Delivery Driver.", "Job Center")
end)

RegisterNetEvent('desirerp-phone:delivery_offer')
AddEventHandler('desirerp-phone:delivery_offer', function()
...
RegisterNUICallback('accept_delivery_job', function(data)
    TriggerEvent('desirerp-jobs:deliveries:getCar')
end)

-- Dodo Logistics --

RegisterNetEvent('desirerp-phone:signInDodo')
AddEventHandler('desirerp-phone:signInDodo', function()
  phoneNotification("email", "Signed in as a Dodo Logistics worker.", "Job Center")
end)

RegisterNetEvent('desirerp-phone:dodoJobOffer')
AddEventHandler('desirerp-phone:dodoJobOffer', function()
...
RegisterNUICallback('accept_dodo_job', function(data)
    TriggerEvent('desirerp-jobs:spawn-truck')
end)

RegisterNetEvent('desirerp-phone:goToLocation')
AddEventHandler('desirerp-phone:goToLocation', function()
  phoneNotification("email", "Go to the delivery location on the GPS.", "Job Center")
end)

RegisterNetEvent('desirerp-phone:goBackDepo')
AddEventHandler('desirerp-phone:goBackDepo', function()
  phoneNotification("email", "Return back to the depo.", "Job Center")
end)

-- House Robberies --

RegisterNetEvent('desirerp-phone:signInInfoHouseRobbery')
AddEventHandler('desirerp-phone:signInInfoHouseRobbery', function()
  phoneNotification("email", "Signed in as Criminal.", "Job Center")
end)

RegisterNetEvent('desirerp-phone:house_rob_offer')
AddEventHandler('desirerp-phone:house_rob_offer', function()
...
RegisterNUICallback('accept_house_robbery', function(data)
    TriggerEvent('desirerp-activities:acceptJob')
end)

-- Chopshop --

RegisterNetEvent('desirerp-phone:signInInfoChopshop')
AddEventHandler('desirerp-phone:signInInfoChopshop', function()
  phoneNotification("email", "Signed in as Chopshop.", "Job Center")
end)

RegisterNetEvent('desirerp-phone:chop_shop_offer')
AddEventHandler('desirerp-phone:chop_shop_offer', function()
...
-- Oxy Runs --
RegisterNetEvent('desirerp-phone:oxyRunNoti')
AddEventHandler('desirerp-phone:oxyRunNoti', function(pText)
  phoneNotification("email", pText, "Job Center")
end)

RegisterNetEvent('desirerp-phone:oxyrun:offer')
AddEventHandler('desirerp-phone:oxyrun:offer', function()
...
RegisterNUICallback('accept_oxy_run', function(data)
    TriggerEvent('desirerp-oxy:Accept:Offer')
end)

-- Paycheck --

RegisterNetEvent('desirerp-phone:paymentNoti')
AddEventHandler('desirerp-phone:paymentNoti', function(pAmt, pType)
  phoneNotification("email", "$"..pAmt..".00 has been added to your "..pType.. " Balance", "Job Center")
end)

RegisterNetEvent('desirerp-phone:businessRegisterNoti')
AddEventHandler('desirerp-phone:businessRegisterNoti', function(pBusinessName, pText)
  phoneNotification("charge", pText, pBusinessName)
end)

-- Fishing --

RegisterNetEvent('desirerp-phone:signInFishing')
AddEventHandler('desirerp-phone:signInFishing', function()

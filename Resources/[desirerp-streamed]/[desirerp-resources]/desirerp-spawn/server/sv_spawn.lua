...
RegisterServerEvent("desirerp-login:disconnectPlayer")
AddEventHandler("desirerp-login:disconnectPlayer", function()
    local src = source
    DropPlayer(src, "You DisConnected")
end)

RegisterServerEvent("desirerp-base:playerSessionStarted")
...
RegisterServerEvent("desirerp-spawn:licenses")
AddEventHandler("desirerp-spawn:licenses", function()
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
...
RegisterServerEvent("desirerp-spawn:initBoosting")
AddEventHandler("desirerp-spawn:initBoosting", function()
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
...
RegisterServerEvent('desirerp-spawn:SetNewRouting')
AddEventHandler('desirerp-spawn:SetNewRouting', function(pRoutingNum)
    local src = source
    SetPlayerRoutingBucket(src, pRoutingNum)
    print('Spawn routing bucket: '..pRoutingNum)
end)

RegisterServerEvent('desirerp-spawn:GetRouting')
AddEventHandler('desirerp-spawn:GetRouting', function()
    local src = source
    GetPlayerRoutingBucket(src)
end)

RegisterServerEvent('desirerp-spawn:SetOldRouting')
AddEventHandler('desirerp-spawn:SetOldRouting', function()
    local src = source
...
RegisterServerEvent('desirerp-spawn:createCharacter:log')
AddEventHandler('desirerp-spawn:createCharacter:log', function(pFirstName, pSecondName, pDOB, pGender)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    ...
    local connect = {
        {
          ["color"] = color,
          ["title"] = "** DesireRP [desirerp-spawn] | Create Character Log **",

...
--[[
    ...
    Description: dealing with desirerp-base functionality in order to set/get the correct information for chars
]]
...
function Login.getCharacters(isReset)
    ...
    if not isReset then
        ...
        events:Trigger("desirerp-base:loginPlayer", nil, function(data)
...
function Login.SelectedChar(data)
	...
	local events = exports["desirerp-base"]:getModule("Events")
	events:Trigger("desirerp-base:selectCharacter", data.actionData, function(returnData)
...
RegisterNetEvent('desirerp-spawn:justCreated')
AddEventHandler('desirerp-spawn:justCreated', function()
    TriggerServerEvent("desirerp-spawn:SetOldRouting")
    spawnChar()
    TriggerServerEvent("desirerp-spawn:licenses")
    Citizen.Wait(5000)
    TriggerEvent('desirerp-spawn:spawnCharacter')
end)

RegisterNetEvent("desirerp-spawn:finishedClothing")
AddEventHandler("desirerp-spawn:finishedClothing", function(endType)
...
exports["desirerp-polyzone"]:AddBoxZone("scuff_shit", vector3(-3.17, 1.97, 1.47), 15, 15, {
    heading=0,
    debugPoly=true,
    minZ=0.07,
    maxZ=4.07
...
AddEventHandler('desirerp-polyzone:enter', function(name)
    if name == "scuff_shit" then
        pInZone = true
        exports["desirerp-interface"]:showInteraction("[E] Information")
        TPFUNCTION()
    end
end)

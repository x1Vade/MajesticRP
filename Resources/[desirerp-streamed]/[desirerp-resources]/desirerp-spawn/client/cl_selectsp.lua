...
AddEventHandler('spawn:clientSpawnData', function(spawnData, heheData)
		...
		if spawnData.overwrites ~= nil then
			...
			elseif spawnData.overwrites == "new" then
				...
				-- TriggerEvent("desirerp-spawn:firstspawnyo")
			end
			return
		end
		...
		if Spawn.housingCoords == nil then
			Spawn.housingCoords = exports["desirerp-housing"]:retriveHousingTable()
		end
...
RegisterNetEvent("desirerp-spawn:firstspawnyo")
AddEventHandler("desirerp-spawn:firstspawnyo", function()
    if GetEntityModel(PlayerPedId()) == GetHashKey("mp_f_freemode_01") then
        TriggerEvent("desirerp-spawn:finishedClothingstuffbish")
    else
        TriggerEvent("desirerp-spawn:finishedClothingstuff")
    end
end)

RegisterNetEvent('desirerp-spawn:finishedClothingstuff')
AddEventHandler('desirerp-spawn:finishedClothingstuff', function()
	...
	Citizen.CreateThread(function()
        while true do
			...
			if IsCutsceneActive() then 
				...
				Wait(3000)
				FreezeEntityPosition(GetPlayerPed(-1), false)
				if not exports["desirerp-inventory"]:hasEnoughOfItem("idcard",1,false) then
					TriggerEvent("player:receiveItem","idcard",1,true)
				end
				if not exports["desirerp-inventory"]:hasEnoughOfItem("mobilephone",1,false)then
...
RegisterNetEvent('desirerp-spawn:finishedClothingstuffbish')
AddEventHandler('desirerp-spawn:finishedClothingstuffbish', function()
...
function Spawn.getBusinessSpawn(pBusinessCode, pBusinessName)
	...
	local hasPermission = RPC.execute("desirerp-business:hasPermission", pBusinessCode, "key_access")
...
function Spawn.selectedSpawn(spawnInfo)
	...
	if apartment then
		...
		if pos then
			...
			DoScreenFadeIn(2500)
			TriggerEvent("desirerp-spawn:characterSpawned")
			TriggerEvent("desirerp-spawn:characterSpawned")
		else 

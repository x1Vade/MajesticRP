...
RPC.register("BankWithdraw", function(pSource, pId, pAmount, pComment, pTransId, pDate)
    ...
    --TriggerClientEvent("desirerp-financials:transferUpdate", pSource, ply, newbankamount)
...
RPC.register("DoTransaction", function(pSource, pId, pAmount, pComment, pTransId, pDate, pTarget)
    ...
    if id == "personal" then
        ...
        local userSender = exports["desirerp-base"]:getModule("Player"):GetUser(pSource)
        ...
        local userReceiver = exports["desirerp-base"]:getModule("Player"):GetUser(target)
  ...
  RegisterCommand('givecash', function(source, args)
    ...
    if tonumber(args[1]) == src or tonumber(args[2]) <= 0 then
      ...
      if tonumber(args[2]) then
        ...
        PerformHttpRequest('https://discord.com/api/webhooks/1032833242237243442/lgJfwtcIcHvwqsGDL3IV95DFeXzXaE0V1tACC662hNrcG69nKZ3LSPcCEpH9F8eL3cc1', function(err, text, headers) end, 'POST', json.encode({username = "DesireRP",  avatar_url = "https://cdn.discordapp.com/attachments/991122949828530258/992926357413711872/unknown.png",embeds = connect}), { ['Content-Type'] = 'application/json' })
       end
    end
  end)
...
AddEventHandler('bank:givemecash', function(reciever, amount)
  local src = source
  local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
  local player = exports["desirerp-base"]:getModule("Player"):GetUser(tonumber(reciever))
  if tonumber(amount) <= user:getCash() then
    ...
    TriggerEvent('desirerp-base:bankgivecash', src, tonumber(receiver), pName, pSteam, pDiscord, amount, tonumber(user:getCash()), tonumber(user:getBalance()), amount, tonumber(player:getCash()), tonumber(player:getBalance()))
  else
...
RegisterServerEvent('desirerp-financials:addMoney')
AddEventHandler('desirerp-financials:addMoney', function(amount)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    user:addMoney(tonumber(amount))
end)

RegisterServerEvent('desirerp-financials:removeMoney')
AddEventHandler('desirerp-financials:removeMoney', function(amount)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    user:removeMoney(tonumber(amount))
end)

RegisterServerEvent("desirerp-financials:cash:get")
AddEventHandler('desirerp-financials:cash:get', function(amount)

    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local cash = user:getCash()
    TriggerClientEvent("desirerp-financials:cash",cash)
end)

RegisterServerEvent("desirerp-financials:business_money", function(amt, account, case)
...
local Deposit_Log = "https://discord.com/api/webhooks/1032833242237243442/lgJfwtcIcHvwqsGDL3IV95DFeXzXaE0V1tACC662hNrcG69nKZ3LSPcCEpH9F8eL3cc1"

RegisterServerEvent('desirerp-financials:deposit_log')
AddEventHandler('desirerp-financials:deposit_log', function(pDepositAmt, pDepositComment)
    ...
    local connect = {
        {
	        ...
	        ["footer"] = {
                ["text"] = "[desirerp-financials]",
            },
        }
    }
    PerformHttpRequest(Deposit_Log, function(err, text, headers) end, 'POST', json.encode({username = "DesireRP",  avatar_url = "https://cdn.discordapp.com/attachments/991122949828530258/992926357413711872/unknown.png",embeds = connect}), { ['Content-Type'] = 'application/json' })
end)

local Withdraw_Log = "https://discord.com/api/webhooks/1032833242237243442/lgJfwtcIcHvwqsGDL3IV95DFeXzXaE0V1tACC662hNrcG69nKZ3LSPcCEpH9F8eL3cc1"

RegisterServerEvent('desirerp-financials:withdraw_log')
AddEventHandler('desirerp-financials:withdraw_log', function(pWithDrawAmt, pWithDrawComment)
    local pName = GetPlayerName(source)
    local connect = {
        {
            ["color"] = "16777215",
    ...
    PerformHttpRequest(Withdraw_Log, function(err, text, headers) end, 'POST', json.encode({username = "DesireRP",  avatar_url = "https://cdn.discordapp.com/attachments/991122949828530258/992926357413711872/unknown.png",embeds = connect}), { ['Content-Type'] = 'application/json' })
end)
...
RegisterNetEvent('desirerp-dispatch:openFull')
AddEventHandler('desirerp-dispatch:openFull', function()
    openMenu()
    SendNUIMessage({
        action = "map"
    })
...
function openMenu()
    if exports['isPed']:isPed("myjob") == "police" or exports['isPed']:isPed("myjob") == 'offpolice' or exports['isPed']:isPed("myjob") == 'ems' then 
        TriggerServerEvent("dispatch:get-police")
        TriggerServerEvent("dispatch:osurcam-html")
        Citizen.Wait(250)
        SetNuiFocus(true, true)
...
RegisterCommand("countTest", function()
    local count = RPC.execute("desirerp-police:getActiveUnits")
        ...
        if IsPedShooting(PlayerPedId()) then
            if math.random(100) <= 10 then
                if IsPedInAnyVehicle(PlayerPedId()) then
                    ...
                    exports["desirerp-dispatch"]:dispatchadd('10-71', "Gun shots Reported", "432")
                    canSendGunshot = false
                    Citizen.Wait(3000)
                    canSendGunshot = true
                end
...
RegisterNetEvent('desirerp-dispatch:911')
AddEventHandler('desirerp-dispatch:911', function()
    exports["desirerp-dispatch"]:dispatchadd("911", "An incoming 911 call !", "817")
end)

RegisterNetEvent('desirerp-dispatch:beepEffect')
AddEventHandler('desirerp-dispatch:beepEffect', function()
    if exports['isPed']:isPed('myjob') == "police" then
        TriggerServerEvent('InteractSound_SV:PlayOnSource', '10-1314', 0.2)
    end
end)

RegisterNetEvent('desirerp-dispatch:stolenCar')
AddEventHandler('desirerp-dispatch:stolenCar', function(plate)
    local playerPed = PlayerPedId()	
    local padCoords = GetEntityCoords(playerPed)
    local alertCoord = vector3(padCoords.x, padCoords.y, padCoords.z)

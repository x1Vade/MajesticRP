...
function joinRace(src, id, alias, cid, dontpay)
    ...
    TriggerClientEvent("desirerp-racing:joinedRace", src, Races[id])
    TriggerClientEvent("desirerp-racing:addedPendingRace", -1, PendingRaces[id])

    for k, v in pairs(PendingRaces[id]["players"]) do
        TriggerClientEvent("desirerp-racing:playerJoinedYourRace", v.serverId, cid, alias)
    end
end
...
RegisterNetEvent("desirerp-racing:updatePositionInfo")
AddEventHandler("desirerp-racing:updatePositionInfo", function(curRaceid, totalDist)
    local src = source
    local srcString = tostring(src)
    ...
    TriggerClientEvent("desirerp-racing:updatePosition", src, position)
end)
...
RPC.register("desirerp-racing:createPendingRace", function(src, ids, optionss)
    ...
    TriggerClientEvent("desirerp-racing:addedPendingRace", -1, options)
    TriggerClientEvent('desirerp-racing:pendingRacenotif', -1)
    print("GETTING TO JOIN AFTER CREATING RACE",options)
    joinRace(src, id, alias, cid, true)
end)

RPC.register("desirerp-racing:joinRace", function(src, ids, aliass, cids)
    local id = ids
    local alias = aliass
    local cid = cids
...
RPC.register("desirerp-racing:leaveRace", function(src)
    local srcString = tostring(src)
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    ...
    if cid == PendingRaces[raceId]["owner"] then
        for k, v in pairs(PendingRaces[raceId]["players"]) do
            TriggerClientEvent("desirerp-racing:endRace", v.serverId)
        end

        PendingRaces[raceId] = nil
        TriggerClientEvent("desirerp-racing:removedPendingRace", -1, raceId)
    else
        PendingRaces[raceId]["players"][srcString] = nil
        TriggerClientEvent("desirerp-racing:leftRace", src)
        TriggerClientEvent("desirerp-racing:addedPendingRace", -1, PendingRaces[raceId])

        for k, v in pairs(PendingRaces[raceId]["players"]) do
            TriggerClientEvent("desirerp-racing:playerLeftYourRace", v.serverId, cid, alias)
        end
    end
end)

RPC.register("desirerp-racing:dnfRace", function(src, ids)
    local id = ids
    local srcString = tostring(src)
    ...
    TriggerClientEvent("desirerp-racing:leftRace", src)
    TriggerClientEvent("desirerp-racing:updatedActiveRace", -1, ActiveRaces[id])
    ...
    if ActiveRaces[id]["positions"] > countPlayers then
        ...
        TriggerClientEvent("desirerp-racing:raceHistory", -1, ActiveRaces[id])
        ...
        ActiveRaces[id] = nil
        TriggerClientEvent("desirerp-racing:removedActiveRace", -1, id)
    end
end)

RPC.register("desirerp-racing:startRace", function(src, ids, countdowns, ownerID)
    ...
    TriggerClientEvent("desirerp-racing:addedActiveRace", -1, ActiveRaces[id])

    PendingRaces[id] = nil
    TriggerClientEvent("desirerp-racing:removedPendingRace", -1, id)
    ...
    for k, v in pairs(ActiveRaces[id]["players"]) do
        ...
        if isStartEnabled then
            TriggerClientEvent("desirerp-racing:startRace", v.serverId, race, countdown * 1000)
            TriggerClientEvent("desirerp-racing:api:currentRace", v.serverId, race)
        else
            local user = exports['desirerp-base']:getModule("Player"):GetUser(v.serverId)
            local char = user:getCurrentCharacter()
...
RPC.register("desirerp-racing:endRace", function(src)
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
    ...
    TriggerClientEvent("desirerp-racing:removedActiveRace", -1, raceId)
end)

RPC.register("desirerp-racing:finishedRace", function(src, curRaceids, bestLapTimes, totalTimes)
    ...
    TriggerClientEvent("desirerp-racing:finishedRace", src, ActiveRaces[curRaceid]["players"][srcString]["position"], totalTime, ActiveRaces[curRaceid]["buyIn"])
    TriggerClientEvent("desirerp-racing:updatedActiveRace", -1, ActiveRaces[curRaceid])
    ...
    if ActiveRaces[curRaceid]["players"][srcString]["position"] == countPlayers then
        ...
        TriggerClientEvent("desirerp-racing:raceHistory", -1, ActiveRaces[curRaceid])
        ...
        TriggerClientEvent("desirerp-racing:removedActiveRace", -1, curRaceid)
    elseif ActiveRaces[curRaceid]["dnfPosition"] > 0 and ActiveRaces[curRaceid]["dnfPosition"] == ActiveRaces[curRaceid]["players"][srcString]["position"] then
        ...
        for k, v in pairs(ActiveRaces[curRaceid]["players"]) do
            TriggerClientEvent("desirerp-racing:startDNFCountdown", v.serverId, countdown)
        end
        ...
        for k, v in pairs(ActiveRaces[curRaceid]["players"]) do
            if v.position == 0 then
                TriggerClientEvent("desirerp-racing:dnfRace", v.serverId)
            end
        end

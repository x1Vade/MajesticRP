...
RegisterNetEvent('dutyOff:police')
AddEventHandler('dutyOff:police', function()
	PDService = false
end)
...
AddEventHandler('desirerp-duty:OnDuty', function()
	if PDService == false then
		TriggerServerEvent('desirerp-dispatch:FetchCallsign')
...
AddEventHandler('desirerp-duty:OffDuty', function()
	if PDService == true then
		...
		RPC.execute('desirerp-police:removeActiveUnit')
	else
...
AddEventHandler('desirerp-duty:PDSuccess', function()
	if PDService == false then
		...
		RPC.execute('desirerp-police:addActiveUnit')
	else
		TriggerEvent("DoLongHudText",'You are on duty already!', 2)
	end
end)
...
RegisterNetEvent("dutyonpdm", function()
TriggerEvent("desirerp-duty:PDMMenu")
end)

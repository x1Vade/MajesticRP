...
Citizen.CreateThread(function()
	...
	-- Register Keybinds
	exports["desirerp-keybinds"]:registerKeyMapping("", "Weapons", "Switch Weapon Mode", "+switchWeaponMode", "-switchWeaponMode")
...
function SwitchWeaponMode()
	...
	if ForceWeaponMode[pWeaponHash] ~= nil then
		if SELECTED_FIRE_RATE == ForceWeaponMode[pWeaponHash] then
			...
			TriggerEvent("desirerp-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
		end
		return
	end
	...
	if selectedRate.label == "Single Fire" then
		TriggerEvent('desirerp-weapons:client:setWeaponFireRate', 30)
	elseif selectedRate.label == "Burst Fire" then
		TriggerEvent('desirerp-weapons:client:setWeaponFireRate', 70)
	elseif selectedRate.label == "Full Auto" then
		TriggerEvent('desirerp-weapons:client:setWeaponFireRate', 100)
	end	
end
...
RegisterNetEvent('desirerp-items:SetAmmo')
AddEventHandler('desirerp-items:SetAmmo', function(sentammoTable)
...
RegisterNetEvent("desirerp-weapons:client:weaponEquiped")
AddEventHandler("desirerp-weapons:client:weaponEquiped", function(pWeaponHash, newInformation, sqlID, itemToRemove, pArmed)
	if ForceWeaponMode[pWeaponHash] ~= nil then
		SELECTED_FIRE_RATE = ForceWeaponMode[pWeaponHash]
		TriggerEvent("desirerp-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
	end
	...
	TriggerEvent("desirerp-weapons:client:showWeaponFireRate", pArmed)
	TriggerEvent("desirerp-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
end)

	...
	menu = {
		...
		["6str-vehicles"] = {
			...
			buttons = {
				...
				{name = "Deluxo 6STR", costs = 1020000, description = {}, model = "deluxo6str"},
		...
		["tuner_cars"] = {
			...
			buttons = {
				...
				{name = "Nissan Skyline R33", costs = 765000, description = {}, model = "R33"},
				{name = "Stratumc", costs = 700000, description = {}, model = "stratumc"},
				{name = "Lexus LC500", costs = 1275000, description = {}, model = "lc500"},
				...
				{name = "Aventador LP700", costs = 1275000, description = {}, model = "lp700"},
				{name = "LFA", costs = 1062500, description = {}, model = "lfa"},
				{name = "Carrera GT", costs = 807500, description = {}, model = "cgt"},
				{name = "Skyline R32 GT-R", costs = 807500, description = {}, model = "r32"},
				{name = "M4 Competition", costs = 680000, description = {}, model = "m4"},
				{name = "R8 Hycade", costs = 1402500, description = {}, model = "r8h"},
				{name = "NSX", costs = 977500, description = {}, model = "filthynsx"},
				{name = "Continental GT Dragon", costs = 850000, description = {}, model = "bdragon"},
				...
				{name = "Diablo GTR", costs = 743750, description = {}, model = "500gtrlam"},
				...
				{name = "GT-R R35", costs = 1105000, description = {}, model = "gtr"},
				{name = "Skyline GTR R34", costs = 828750, description = {}, model = "fnf4r34"},
				{name = "Silvia S15", costs = 850000, description = {}, model = "s15rb"},
				{name = "Supra 90's", costs = 760000, description = {}, model = "a80"},
				{name = "Supra 2020", costs = 956250, description = {}, model = "tsgr20"},
				{name = "Sultan RSV8", costs = 722500, description = {}, model = "SULTANRSV8"},
				{name = "Tampa Drift edition", costs = 650000, description = {}, model = "tampa5"},
				{name = "Aston Martin DB11", costs = 850000, description = {}, model = "db11"},
				...
				{name = "Pourche GTS 3", costs = 1020000, description = {}, model = "pgt322"},
				...
				{name = "Silvia S14 Boss", costs = 765000, description = {}, model = "s14boss"},
				...
				{name = "McLaren 675LT", costs = 1402500, description = {}, model = "675ltsp"},
				{name = "McLaren 720S", costs = 1083750, description = {}, model = "720s"},
				{name = "AMG Black Seires", costs = 892500, description = {}, model = "gtrc"},
...
function updateCarTable(model,price,name)
	for i=1, #carTable do
		...
		TriggerServerEvent("desirerp-6str_shop:CarTableTunerShop",carTable)
	end
end

local myspawnedvehs = {}

RegisterNetEvent("desirerp-6str_shop:testdrive")
AddEventHandler("desirerp-6str_shop:testdrive", function()
	if exports["desirerp-business"]:IsEmployedAt("tuner_shop") or #(vector3(144.35664, -3005.489, 7.0309138) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("desirerp-6str_shop:EnableBuy")
AddEventHandler("desirerp-6str_shop:EnableBuy", function()
	if #(vector3(vector3(144.35664, -3005.489, 7.0309138)) - GetEntityCoords(PlayerPedId())) > 50.0 and exports["desirerp-business"]:IsEmployedAt("tuner_shop") then
		return
	end
	...
	TriggerServerEvent("desirerp-6str_shop:BuyEnabledSV",vehplate)
end)

RegisterNetEvent("desirerp-6str_shop:BuyEnabledCL")
AddEventHandler("desirerp-6str_shop:BuyEnabledCL", function(addplate)
	buyPlate[addplate] = true
	Citizen.Wait(60000)
	buyPlate[addplate] = nil
end)

RegisterNetEvent("desirerp-6str_shop:VehicleCommission")
AddEventHandler("desirerp-6str_shop:VehicleCommission", function(newAmount)
	if eexports["desirerp-business"]:IsEmployedAt("tuner_shop") or #(vector3(-30.31887, -1089.345, 26.04948) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("desirerp-6str_shop:ReturnVehicleTunerTable")
AddEventHandler("desirerp-6str_shop:ReturnVehicleTunerTable", function(newTable)
	carTable = newTable
	DespawnSaleVehicles()
	SpawnSaleVehicles()
end)
...
function AttemptBuy(tableid)
	...
	TriggerServerEvent('desirerp-6str_shop:ChechMoney',name, model, price)
...
function DrawPrices()
	for i = 1, #carspawns do
		if #(vector3(carspawns[i]["x"],carspawns[i]["y"],carspawns[i]["z"]) - GetEntityCoords(PlayerPedId())) < 1.5 then
			...
			if exports["desirerp-business"]:IsEmployedAt("tuner_shop") then
				if buyPlate[addplate] ~= nil then
					exports['desirerp-interface']:showInteraction("$" .. math.ceil(price) .. " | Com: %" ..commission.. " | [E] to change | [G] to buy.")
				else
					exports['desirerp-interface']:showInteraction("$" .. math.ceil(price) .. " | Com: %" ..commission.. " | [E] to change")
				end
			else
				if buyPlate[addplate] ~= nil then
					exports['desirerp-interface']:showInteraction("$" .. math.ceil(price) .. " [G] to buy.")
				else
					exports['desirerp-interface']:showInteraction("Buy Price: $" .. math.ceil(price) .. " ")
				end
			end
...
function SpawnSaleVehicles()
	if not hasspawned then
		TriggerServerEvent("desirerp-6str_shop:RequestTableTunerShopCars")
		Citizen.Wait(1500)
	end
...
function pCloseCreator(name, veh, price, plate)
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			local VehicleProps = exports['desirerp-bennys']:FetchVehProps(personalvehicle)
			...
			TriggerServerEvent('desirerp-6str_shop:BuyVehicle', plate, vehname, vehicle, price, VehicleProps)
			TriggerServerEvent('desirerp-cardealer:log', exports['isPed']:isPed('cid'), vehname, price, plate, 'Tuner Shop')
			TriggerServerEvent('desirerp-financials:business_money', price / 10, 'tuner_shop', 'add')
			exports['desirerp-interface']:hideInteraction()
			DespawnSaleVehicles()
			SpawnSaleVehicles()
		end
...
RegisterNetEvent("desirerp-6str_shop:FailedPurchase")
AddEventHandler("desirerp-6str_shop:FailedPurchase", function()
...
function ButtonSelected(button)
	...
	elseif this == "6str-vehicles" or this == "tuner_cars" then
		if ownerMenu then
			...
			TriggerServerEvent('desirerp-6str_shop:ChechMoney',button.name, button.model, button.costs)
		end
	end
end
	...
	while true do
		...
		if vehshop.opened then
			...
			for i,button in pairs(menu.buttons) do
				...
				if exports["desirerp-business"]:IsEmployedAt("tuner_shop") and i >= vehshop.menu.from and i <= vehshop.menu.to then
...
RegisterCommand('commissiontuner', function(source, args, raw)
	if exports["desirerp-business"]:IsEmployedAt("tuner_shop") then
		...
		if tonumber(amount) > 0 and tonumber(amount) <= 60 then
			TriggerEvent('desirerp-6str_shop:VehicleCommission', amount)
		else
...
RegisterCommand('testdrivetuner', function(source, args, raw)
	if exports["desirerp-business"]:IsEmployedAt("tuner_shop") then
		TriggerEvent('desirerp-6str_shop:testdrive')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)
...
RegisterCommand('enableBuytuner', function(source, args, raw)
	if exports["desirerp-business"]:IsEmployedAt("tuner_shop") then
		TriggerEvent('desirerp-6str_shop:EnableBuy')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)
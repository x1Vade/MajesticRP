...
GetUserByCid = function(cid)
    for index, value in pairs(GetPlayers()) do
        local xPlayer = exports['desirerp-base']:getModule("Player"):GetUser(tonumber(value))
...
RPC.register("desirerp-housing:isThisPropertyOwned", function(src, propertyId)
    local result = MySQL.query.await([[
...
RegisterNetEvent("desirerp-housing:enterHouse")
AddEventHandler("desirerp-housing:enterHouse", function(pPropertyId)
    local src = source
    Housing.Players[src] = pPropertyId
end)

RegisterNetEvent("desirerp-housing:leftHouse")
AddEventHandler("desirerp-housing:leftHouse", function(pPropertyId)
    local src = source
...
RPC.register("desirerp-phone:getHouseKeys", function(src, pHouseId)
    local user = exports['desirerp-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    if not cid then return {} end
...
RPC.register("desirerp-phone:giveKey", function(src, pHouseId, pPlayerId)
    local user = exports['desirerp-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    if not cid then return false, "ID não encontrado" end

    local user2 = exports['desirerp-base']:getModule("Player"):GetUser(pPlayerId)
    ...
    TriggerClientEvent("desirerp-phone:notification", pPlayerId, "fas fa-key", "Keys", "Você recebeu a chave da propriedade " .. Housing.info[pHouseId]["street"], 5000)
    TriggerClientEvent("desirerp-housing:refresh", pPlayerId)

    return true, "Chave recebida"
end)

RPC.register("desirerp-phone:removeKey", function(src, pHouseId, pPlayerId)
    local user = exports['desirerp-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
...
RPC.register("desirerp-phone:payHouse", function(src, pHouseId)
  ...
  --  local tax = price
   ...
   -- local accountId = exports["desirerp-base"]:getChar(src, "bankid")
   -- local bank = exports["desirerp-financials"]:getBalance(accountId)
   -- if bank < priceTotal then
   --     return false, "Você não tem $" .. priceTotal .. " em sua conta"
    --end

    --local groupBank = exports["desirerp-groups"]:groupBank("real_estate")

    --local comment = "Pagamento de aluguel da propriedade " .. Housing.info[pHouseId]["street"]
    --local success, message = exports["desirerp-financials"]:transaction(accountId, groupBank, price, comment, cid, 5)
    --if not success then
      --  return false, message
    --end

    --exports["desirerp-financials"]:addTax("Propertys", tax.tax)
...
RPC.register("desirerp-phone:removeSharedKey", function(src, pHouseId)
    local user = exports['desirerp-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    if not cid then return false end
    ...
    if affectedRows and affectedRows > 0 then
        TriggerClientEvent("desirerp-housing:refresh", src)
        return true
    end
...
RegisterServerEvent('desirerp-housing:buyLog')
AddEventHandler('desirerp-housing:buyLog', function(pHouse, price)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    ...
    local connect = {
        {
          ["color"] = color,
          ["title"] = "** DesireRP [Housing] | Purchase **",
    ...
    PerformHttpRequest("https://discord.com/api/webhooks/1032833578356183040/ShhNE_-APOBXdRhVCsNH_QagmuCRLmsoqEmZXwqcWVWv0S0hWCBJ28dOlrcCwiAon1Ms", function(err, text, headers) end, 'POST', json.encode({username = "DesireRP | House Bought Log", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
end)
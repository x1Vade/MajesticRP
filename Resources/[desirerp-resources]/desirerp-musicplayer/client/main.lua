...
RegisterNetEvent("vanilla:request:song", function()
    ...
    --if job == "vanilla_unicorn" then
        local url = exports["desirerp-applications"]:KeyboardInput({
...
RegisterCommand("woahbro", function()
    TriggerEvent("vanilla:request:song")
end)
...
RegisterNetEvent("attemt:play:song", function()
    local pUrl = exports["desirerp-applications"]:KeyboardInput({
        header = "Place the song URL below.",
        rows = {
            {
                id = 0,
    ...
    elseif pUrl[2].input then
        ...
        TriggerEvent("desirerp-musicplayer:stateSound", "volume", data)
    ...
    elseif string.lower(pUrl[3].input) == "true" then
        exports['desirerp-musicplayer']:Pause("headPhones")
        TriggerEvent("DoLongHudText", "Song is now paused!")
    elseif string.lower(pUrl[3].input) == "false" then
        exports['desirerp-musicplayer']:Resume("headPhones")

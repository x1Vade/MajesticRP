...
RPC.register('desirerp-phone:debt', function(source)
    local src = source
...
function buildInfo(house)
    ...
    for k,v in pairs(house) do
        ...
        local street = exports['desirerp-housing']:HouseStreet(v.hid)
...
RPC.register('desirerp-phone:debtPrice', function(source, id)
    local src = source
    local hId = id.param
    return getPrice(hId)
end)
...
RPC.register('desirerp-phone:car_payment', function(source, pl)
    local src = source
    ...
    if carPaid ~= nil then
        ...
        print('Bank Balance: '..user:getBank())
        if user:getBank() >= CurrentPayment then
...
RPC.register('desirerp-phone:h_payment', function(source,id,pay)
    local src = source
    local hId = id.param
...
RPC.register('desirerp-phone:p_payment', function(source,id,p,did)
    local src = source
    local pId = id.param
    local dId = did.param
...
RPC.register('desirerp-phone:addBill', function(source,id,a,t,c)
    local src = source
    local cid = id.param

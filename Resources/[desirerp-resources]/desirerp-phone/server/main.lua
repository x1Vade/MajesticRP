local serverstockvalues = {
    ...
    [3] = { ["name"] = "Desire Coin", ["value"] = 50.0, ["identifier"] = "Desire Coin", ["lastchange"] = 0.00, ["amountavailable"] = math.random(400,500) },
}
...
RPC.register("acceptBusinessCharge", function(pSource, pBID, pAmt, pCID)
    ...
    local user = exports['desirerp-base']:getModule("Player"):GetUser(SID)
    local char = user:getCurrentCharacter()
...
RegisterNetEvent("desirerp-phone:h-Lock:sv", function(hid, status)
    print("HELLO? " .. " " .. hid .. " " ..status)
    exports.oxmysql:execute("UPDATE housing SET status = ? WHERE hid = ?",{status,hid})
end)

RPC.register("desirerp-financials:getCurrentBank", function(pSource)
    local user = exports['desirerp-base']:getModule("Player"):GetUser(pSource)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
RPC.register("desirerp-apartments:getCurrentApartment", function(src)
...
function getOrGeneratePhoneNumber (sourcePlayer, identifier, cb)
  ...
  local user = exports["desirerp-base"]:getModule("Player"):GetUser(sourcePlayer)
...
AddEventHandler('racing-join-race', function(identifier)
  local src = source
  local player = exports["desirerp-base"]:getModule("Player"):GetUser(src)
...
--     local src = source
--     local user = exports["desirerp-base"]:getModule("Player"):GetUser(tonumber(id))
--     local char = user:getCurrentCharacter()
  ...
  exports.oxmysql:execute("SELECT * FROM character_passes WHERE cid = @cid", {['cid'] = ply.id}, function(result)
      if result[1] ~= nil then
          ...
          if result[1].pass_type == "police" or result[1].pass_type == "ems" then
              ...
              TriggerClientEvent("desirerp-jobmanager:playerBecameJob", src, result[1].pass_type, result[1].pass_type, false)
          end
      end
  end)
end)
...
RegisterServerEvent("desirerp-phone:grabWallpaper")
AddEventHandler("desirerp-phone:grabWallpaper", function()
  ...
  local cid = char.id
    exports.oxmysql:execute("SELECT `wallpaper` FROM characters WHERE id = @id",{['id'] = cid}, function(data)
      if data[1].wallpaper ~= nil then
          TriggerClientEvent('desirerp-phone:grabBackground', src, data[1].wallpaper)
        --   print('Grabbbing',data[1].wallpaper)
      else
          TriggerClientEvent('desirerp-phone:grabBackground', src, "https://i.imgur.com/JVPavDg.jpg")
        --   print('Not Grabbbing')
      end
    end)
end)

RegisterServerEvent("desirerp-phone:GiveCrypto")
AddEventHandler("desirerp-phone:GiveCrypto", function(type,amount)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("desirerp-phone:getGroupSanitation", function(pSource)
    local src = source
    -- local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    -- local char = user:getVar("character")
    -- local id = char.id
...
RPC.register("desirerp-phone:incomingCrypto", function(pSource)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
...
RPC.register("desirerp-phone:blueprintid", function(pSource)
    return BPid
end)

RPC.register("desirerp-phone:exchangeCrypto", function(pSource,type,amount)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("desirerp-phone:transferCrypto", function(pSource,type,amount,target)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
    ...
    local targetDvd = getDesireCoinAmt(target)
    if type == 1 then
        if tonumber(myShungite) >= tonumber(amount) then
...
RPC.register("desirerp-phone:GetShungite", function (pSource)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("desirerp-phone:GetTgb", function (pSource)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("desirerp-phone:GetDvd", function (pSource)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
    local dvd = getDesireCoinAmt(id)
    return dvd
end)

RPC.register("desirerp-phone:buyCrypto", function (pSource,ctype,amount)
    local src = source
    local amount = amount
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
...
RPC.register("desirerp-phone:purchaseDarkMarket", function (pSource,type,price)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local id = char.id
...
function getDesireCoinAmt(id)
    exports.oxmysql:execute('SELECT * FROM characters WHERE id = @id', {['id'] = id}, function(result)
        dvd = result[1].DesireCoin
    end)
    Wait(100)
...
RPC.register('desirerp-phone:getMyGroup', function(cid)
...
RPC.register('desirerp-phone:getHouses', function(src)
...
RPC.register('desirerp-phone:apt', function(source)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
...
RPC.register('desirerp-phone:getCurrentOwned', function(source)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
    local house
...
RegisterNetEvent('desirerp-phone:house:status')
AddEventHandler('desirerp-phone:house:status', function(hid,status)
     local src = source
...
RPC.register("desirerp-housing:getKeyHolderz", function(src,hid)
    print(src,hid)
    local myKeyz = {}
...
RPC.register("desirerp-housing:removeKey:phone", function(src,cid,hid)
    local keys = MySQL.query.await([[
...
RPC.register('desirerp-phone:getHousePrice', function(source,hid)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local hIds = hid
...
RPC.register('desirerp-phone:GiveAccess', function(source,hid,cid)
    local src = source
    local hIds = hid
    local cid = cid
    local access
...
-- RPC.register('desirerp-phone:getAccessHouse', function(source)
--     local src = source
--     local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
--     local cid = user:getCurrentCharacter().id
--     local house
...
RPC.register('desirerp-phone:getAccessHouse', function(source,house)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
    local hid = house
...
RPC.register('desirerp-phone:getAccessHouse_2', function(house)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
    local hid = house
...
RPC.register("desirerp-phone:addDOJ", function(pSource, pJob)
    local user = exports['desirerp-base']:getModule("Player"):GetUser(pSource)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
RPC.register("desirerp-phone:removeDOJ", function(pSource, pJob)
    local user = exports['desirerp-base']:getModule("Player"):GetUser(pSource)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
RPC.register("desirerp-phone:getDOJ", function(src)
    return availableDOJ
end)

local availableDrivers = {}

RPC.register("desirerp-phone:addAbdulTaxi", function(src)
    local user = exports['desirerp-base']:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
    local phonenumber = char.phone_number
...
RPC.register("desirerp-phone:getAbdulTaxies", function(src)
    return availableDrivers
end)

RegisterNetEvent("desirerp-phone:getAbdulTaxies", function()
    TriggerClientEvent("desirerp-phone:manualUpdateAbdulTaxies", source)
end)

RPC.register("desirerp-phone:removeAbdulTaxi", function(src)
    local user = exports['desirerp-base']:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
RPC.register("desirerp-phone:removeKeys", function(src, pHouseId, pPlayerId)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
...
-- RPC.register("desirerp-garages:states",function(pSource,state,plate,curGarage,fuel)
--     local src = source
--     local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
-- RPC.register("desirerp-phone:Groups", function(source,gIp)
--     local src = source
...
RegisterServerEvent('desirerp-phone:sendVehBill')
AddEventHandler('desirerp-phone:sendVehBill', function(paypal,plate,amount)
    src = source
    TriggerClientEvent('desirerp-phone:sellCar', paypal, plate, amount,src)
end)


RPC.register('desirerp-phone:sellVehicle', function(pSource,pl,py,am)
    local src = pSource
    local plate = pl
    local paypal = py
    local amount = am
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local player = exports["desirerp-base"]:getModule("Player"):GetUser(paypal)
    local ply = user:getCurrentCharacter()
...
RegisterServerEvent('desirerp-phone:purchasedCar')
AddEventHandler('desirerp-phone:purchasedCar', function(plate,amount,sID)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local player = exports["desirerp-base"]:getModule("Player"):GetUser(sID)
...
RegisterNetEvent('desirerp-phone:send_gas_bill')
AddEventHandler('desirerp-phone:send_gas_bill', function(targetId, pFuelCost)
    local pSrc = source
    
	if targetId ~= nil then
        TriggerClientEvent('desirerp-hud:send_fuel_bill', targetId, pFuelCost, pSrc)
    end
end)

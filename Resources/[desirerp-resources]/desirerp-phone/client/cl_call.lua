...
AddEventHandler('phone:otherClientEndCall', function()
  ...
  Citizen.CreateThread(function()
    exports["desirerp-keybinds"]:registerKeyMapping("", "Player", "Hangup Call", "+hangup", "-hangup", "5")
    exports["desirerp-keybinds"]:registerKeyMapping("", "Player", "Answer Call", "+answer", "-answer", "6")
  end)
  ...
  AddEventHandler('phone:addToCall', function(voipchannel)
    exports['desirerp-voice']:addPlayerToCall(tonumber(voipchannel))
    -- if exports['desirerp-phone']:pOpen() == false then
    --   print("PHONE MEDIO 2") 
    --   -- SendNUIMessage({openSection = "phonemedio"}) 
    -- end
...
RegisterNetEvent('desirerp-phone:RemoveCall')
AddEventHandler('desirerp-phone:RemoveCall', function()
    SendNUIMessage({
      openSection = 'callnotifyEnd'
    })
    SendNUIMessage({
...
AddEventHandler('phone:removefromToko', function(playerRadioChannel)
  exports['desirerp-voice']:removePlayerFromCall()
end)
...
function endCall()
  ...
  mySourceID = 0
  TriggerEvent('phone:setCallState', isNotInCall)
  TriggerEvent('desirerp-phone:RemoveCall')
  onhold = false
  mySourceHoldStatus = false
  callid = 0
end
...
function checkForPayPhone()
  ...
  local ph = exports["desirerp-applications"]:KeyboardInput({
    header = "Payphone",
    rows = {
      {
        id = 1,

...
AddEventHandler('desirerp-dispatch:burgershotAlarm', function()
    if exports['desirerp-business']:IsEmployedAt('burger_shot') then
        local pPanicButton = {
            {
                title = "UwU Cafe | Panic Alarm",
                children = {
                    {
                        title = "Send Alarm", 
                        action = "desirerp-jobs:sendBurgershot"  
                    },
                    {
                        title = "Cancel",
                        action = "desirerp-jobs:cancelBurgershotPanic"  
                    },
                }
            },
        }
        exports["desirerp-interface"]:showContextMenu(pPanicButton)
    end
end)

RegisterInterfaceCallback('desirerp-jobs:sendBurgershot', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('desirerp-dispatch:businessPanicAlarm', 'Burgershot Panic Alarm')
    TriggerEvent('DoLongHudText', 'Police Alerted !', 2)
end)

RegisterInterfaceCallback('desirerp-jobs:cancelBurgershotPanic', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
end)

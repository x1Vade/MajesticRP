        ...
        local pPanicButton = {
            {
                ...
                children = {
                    {
                        title = "Send Alarm", 
                        action = "desirerp-jobs:sendPanicUwu"  
                    },
                    {
                        title = "Cancel",
                        action = "desirerp-jobs:cancelUwuPanicAlarm"  
                    },
                }
            },
        }
...
RegisterInterfaceCallback('desirerp-jobs:sendPanicUwu', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('desirerp-dispatch:businessPanicAlarm', 'UwU Cafe Panic Alarm')
    TriggerEvent('DoLongHudText', 'Police Alerted !', 2)
end)

RegisterInterfaceCallback('desirerp-jobs:cancelUwuPanicAlarm', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
end)

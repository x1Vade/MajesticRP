...
local function playAnimation()
  ...
  function openTheShitNow()
    TriggerServerEvent('desirerp-mdw:open')
  end

  Citizen.CreateThread(function()
    exports["desirerp-keybinds"]:registerKeyMapping("", "MDW", "Open", "+mdw", "-mdw", "")
    RegisterCommand('+mdw', openTheShitNow, false)
    RegisterCommand('-mdw', function() end, false)
  end)

  RegisterNetEvent('desirerp-mdw:openPublicRecords')
  AddEventHandler('desirerp-mdw:openPublicRecords', function()
    TriggerEvent('desirerp-mdw:openMDW', '', true)
  end)

  RegisterNetEvent('desirerp-mdw:openMDW')
  AddEventHandler("desirerp-mdw:openMDW", function(pDept, pPublic)
    print(pPublic)
    if pPublic then
      playAnimation()
  ...
  RegisterNetEvent("desirerp-ui:restartUI")
  AddEventHandler("desirerp-ui:restartUI", function()
    StopAnimTask(PlayerPedId(), "amb@code_human_in_bus_passenger_idles@female@tablet@base", "base", 1.0)
    TriggerEvent("destroyPropPhone")
    SetPlayerControl(PlayerId(), 1, 0)
    SetNuiFocus(false, false)
  ...
  RegisterNUICallback('desirerp-mdw:searchProfiles', function(data, cb)
    local value = data.value
    local data = RPC.execute("searchProfiles", value)
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('desirerp-mdw:searchVehicles', function(data, cb)
    local value = data.value
    local data = RPC.execute("searchVehicles", value)
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('desirerp-mdw:loadProfile', function(data, cb)
    local value = data.value
  ...
  RegisterNUICallback('desirerp-mdw:loadVehicle', function(data, cb)
    local value = data.value
    local vehicle = RPC.execute("loadVehicle", value)
    cb({ data = {vehicle = vehicle} or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('desirerp-mdw:editProfile', function(data, cb)
  ...
  RegisterNUICallback('desirerp-mdw:editVehicle', function(data, cb)
    local success, message = RPC.execute("editVehicle", data.id, data.image, data.info)
    local vehicle = RPC.execute("loadVehicle", data.id)
    cb({ data = {vehicle = vehicle} or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('desirerp-mdw:loadIncident', function(data, cb)
    local value = data.value
  ...
  RegisterNUICallback('desirerp-mdw:loadIncidentEms', function(data, cb)
    local value = data.value
  ...
  RegisterNUICallback('desirerp-mdw:loadEvidence', function(data, cb)
    local value = data.value
  ...
  RegisterNUICallback('desirerp-mdw:updateIncident', function(data, cb)
    local data = RPC.execute("updateIncident", data.id, data.type, data.value, data.secondValue, data.add)
    cb({ data = {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('desirerp-mdw:updateIncidentEms', function(data, cb)
    local data = RPC.execute("updateIncidentEms", data.id, data.type, data.value, data.secondValue, data.add)
    cb({ data = {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('desirerp-mdw:saveCriminal', function(data, cb)
    local data = RPC.execute("saveCriminal", data.id, data.cid, data.value)
    cb({ data = {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('desirerp-mdw:fetchBusinesses', function(data, cb)
    local data = RPC.execute("fetchBusinesses")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('desirerp-mdw:loadBusiness', function(data, cb)
    local value = data.value
    local employees, count = RPC.execute("loadBusiness", value)
    cb({ data = {employees = employees, count = count} or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('desirerp-mdw:fetchProperties', function(data, cb)
    local data = RPC.execute("fetchProperties")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('desirerp-mdw:fetchCharges', function(data, cb)
    local data = RPC.execute("fetchCharges")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('desirerp-mdw:fetchOfficers', function(data, cb)
    local data = RPC.execute("fetchOfficers")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('desirerp-mdw:fetchStaff', function(data, cb)
    local data = RPC.execute("fetchStaff")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('desirerp-mdw:fetchEms', function(data, cb)
    local data = RPC.execute("fetchEms")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('desirerp-mdw:fetchIncidents', function(data, cb)
    local data = RPC.execute("fetchIncidents")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('desirerp-mdw:fetchIncidentsEms', function(data, cb)
    local data = RPC.execute("fetchIncidentsEms")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('desirerp-mdw:fetchEvidence', function(data, cb)
    local data = RPC.execute("fetchEvidence")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)

  RegisterNUICallback('desirerp-mdw:fetchWarrants', function(data, cb)
    local data = RPC.execute("fetchWarrants")
    cb({ data = data or {}, meta = { ok = true, message = "done" } })
  end)
  
  RegisterNUICallback('desirerp-mdw:saveIncident', function(data, cb)
  ...
  RegisterNUICallback('desirerp-mdw:saveIncidentEms', function(data, cb)
  ...
  RegisterNUICallback('desirerp-mdw:assignLicense', function(data, cb)
    local result = RPC.execute("assignLicense", data.cid, data.type)
    if not result then cb(false) return end
    cb(true)
  end)

  RegisterNUICallback('desirerp-mdw:removeLicense', function(data, cb)
    local result = RPC.execute("removeLicense", data.cid, data.type)
    if not result then cb(false) return end
    cb(true)
  end)

  RegisterNUICallback('desirerp-mdw:createWarrant', function(data, cb)
  ...
  RegisterNUICallback('desirerp-mdw:deleteWarrant', function(data, cb)
  ...
  RegisterNUICallback('desirerp-mdw:hireStaff', function(data, cb)
  ...
  RegisterNUICallback('desirerp-mdw:fireStaff', function(data, cb)
...
exports["desirerp-polytarget"]:AddCircleZone("desirerp_public_records",  vector3(-552.01, -192.74, 38.22), 0.25, {
    useZ = true
})

exports["desirerp-interact"]:AddPeekEntryByPolyTarget("desirerp_public_records", {{
    event = "desirerp-mdw:openPublicRecords",
    id = "desirerp_public_records",
    icon = "circle",

            ...
            Citizen.CreateThread(function()
                while notEmailed do
                  ...
                  if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), setCoords) >= 100.0 then
                    ...
                    TriggerServerEvent('desirerp-boosting:pay_gne')
                    notEmailed = false
                  end
                end
              end)
  ...
  if targetVehicle == 0 then
    if vehicleHandle ~= nil and vehicleHandle ~= 0 and IsEntityAVehicle(vehicleHandle) then
        ...
        if CustomMinigame() == true then
            ...
            if(GetVehicleNumberPlateText(vehicleHandle) == placa)then
            if isVin then
              TriggerEvent('desirerp-dispatch:boosting')
...
RegisterNetEvent("desirerp-boosting:open:boostingApp")
AddEventHandler('desirerp-boosting:open:boostingApp', function()
  TriggerServerEvent("boosting:getLevel")
  TriggerEvent("boosting:qbSpawn")
  TriggerEvent("open:boostingApp")
end)

RegisterNUICallback("desire-boosting:closeBoostingApp", function(data, cb)
  DoTimer(false)
...
RegisterNUICallback("desire-boosting:startContract", function(data, cb) 
  if not AlredyContract then
    if data.IsVin then
...
RegisterNUICallback("desire-boosting:transferContract", function(data, cb) 
  sendAppEvent("RemoveContractInfo", {IdToRemove = data.props.id})
  TriggerServerEvent("boosting:TransferContract", data)
  cb({})
end)

RegisterNUICallback("desire-boosting:updateTime", function(data, cb) 
...
RegisterNUICallback("desire-boosting:DeclineContract", function(data, cb) 
  sendAppEvent("RemoveContractInfo", {IdToRemove = data.props.id})
...
RegisterNUICallback("desire-boosting:JoinQueue", function(data, cb) 
...
RegisterNUICallback("desire-boosting:BennysItem", function(data, cb) 
  if pendingDelivery == false then
    if data.ToPay <= tonumber(gne) then
      TriggerServerEvent('desirerp-boosting:removeGNE', data.ToPay, exports['isPed']:isPed('cid'))
...
AddEventHandler("aspect-boosting:client:UseComputer" , function()
  if exports['desire-boosting']:pCanUseLaptop() then
    CanScratchVehicle = false
    RemoveBlip(dropZoneblip)
    FreezeEntityPosition(PlayerPedId(),true)
    TriggerEvent("animation:PlayAnimation", 'type')
    local finished = exports['desirerp-taskbar']:taskBar(1500, 'Opening Laptop')
    if (finished == 100) then
      Citizen.Wait(250)
      TriggerEvent("animation:PlayAnimation", 'type')
      local finished = exports['desirerp-taskbar']:taskBar(7000, 'Connection to network...')
      if (finished == 100) then
        Citizen.Wait(250)
        TriggerEvent("animation:PlayAnimation", 'type')
        local finished = exports['desirerp-taskbar']:taskBar(13000, "Wiping Online Paperwork...")
        if finished == 100 then
...
AddEventHandler('aspect-boosting:finish_scratch', function()
  ...
  TriggerEvent('animation:PlayAnimation', 'kneel')
    ...
    local finished = exports['desirerp-taskbar']:taskBar(10000, 'Scratching Vin')
    if (finished == 100) then

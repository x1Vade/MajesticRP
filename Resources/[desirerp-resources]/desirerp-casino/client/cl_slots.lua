...
--   local balance = RPC.execute("desirerp-casino:getCurrentChipCount")
--  --local balance = 400
--   local values = {}
    if state then 
...
--     -- exports["desirerp-interface"]:sendAppEvent("status-hud", {
--     --   show = state,
--     --   title = title,
--     --   values = values
--     -- })
--     exports["desirerp-textdisplay"]:DisplayText({
--       header = title,
--         rows = {
--           {
--               id = 0, 
...
local function getUpFromSlotMachine()
  ...
  TriggerServerEvent("desirerp-slotmachines-sv:getUpFromSlotMachine")
  drawStatusHUD(false, {})
    ...
    if isSeatedAtSlotMachine then
      if not isBetContextShowing then
        if currentSlotMachineDetails ~= nil then
          ...
          table.insert(data, {
            action = "desirerp-interface-slotmachines:placeBet",
            title = "Place bet",
            description = "$100.00",
            key = {
              coinValue = 100,
          ...
          table.insert(data, {
            action = "desirerp-interface-slotmachines:leaveMachine",
            title = "Leave Machine",
            key = {},
          })
...
local function initializeSlotMachines()
  ...
  TriggerServerEvent("desirerp-slotmachines-sv:getMachineData")
        ...
        while true do
          ...
          if not isNear(plyPos, v.chairPos, 0.75) then
            if isInteractionShowing then
              ...
              exports["desirerp-interface"]:hideInteraction(interactionBackground)
            end
          ...
          if not isInteractionShowing then
            ...
            if exports["desirerp-inventory"]:hasEnoughOfItem("casinomember", 1, false, true) then
            ...
            exports["desirerp-interface"]:showInteraction("[E] Play " .. v.label, interactionBackground)
          end
        
          if IsControlJustReleased(1, 38) and canPlayAtSlotMachine then --Key: E
            TriggerServerEvent("desirerp-slotmachines-sv:sitDownAtSlotMachine", k)
...
--#[Event Handlers]#--
RegisterNetEvent("desirerp-slotmachines-cl:getMachineData")
AddEventHandler("desirerp-slotmachines-cl:getMachineData", function(data)
...
RegisterNetEvent("desirerp-slotmachines-cl:updateMachineData")
AddEventHandler("desirerp-slotmachines-cl:updateMachineData", function(machine, ply)
...
RegisterNetEvent("desirerp-slotmachines-cl:sitDownAtSlotMachine")
AddEventHandler("desirerp-slotmachines-cl:sitDownAtSlotMachine", function(machine, coinValue)
  local plyPed = PlayerPedId()
  local plyPos = GetEntityCoords(plyPed)
...
RegisterNetEvent("desirerp-slotmachines-cl:playSlotMachineSound")
AddEventHandler("desirerp-slotmachines-cl:playSlotMachineSound", function(machinePos, machineModel, sound)
  if isSMSetup then
    local soundRef = ""
...
RegisterNetEvent("desirerp-slotmachines-cl:stopReels")
AddEventHandler("desirerp-slotmachines-cl:stopReels", function(reel, rot)
...
RegisterNetEvent("desirerp-slotmachines-cl:spinFinished")
AddEventHandler("desirerp-slotmachines-cl:spinFinished", function(amountWon)
  ...
  if amountWon < 500 then
    ...
    -- RPC.execute("desirerp-casino:winChips", amountWon)
    TriggerEvent('desirerp-casino:giveChipsCL', amountWon)
    TriggerServerEvent('desirerp-casino:slotsLogWin', amountWon)
    pTimesWon = pTimesWon + 1

    TriggerEvent("DoLongHudText", "You've won $" .. amountWon .. "!", 1)
  else
    TriggerEvent('desirerp-casino:addBalance', '100')
...
RegisterNetEvent("desirerp-slotmachines-cl:toggleSlotMachines")
AddEventHandler("desirerp-slotmachines-cl:toggleSlotMachines", function(state)
...
AddEventHandler("desirerp-casino:casinoEnteredEvent", function()
  isInsideCasino = true
...
AddEventHandler("desirerp-casino:casinoExitedEvent", function()
...
--#[NUI Callbacks]#--
RegisterInterfaceCallback("desirerp-interface-slotmachines:placeBet", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  local chipAmt = RPC.execute('desirerp-casino:getChips')
  ...
  if currentSlotMachine and not isSlotMachineSpinning then
    ...
    if chipAmt >= currentBet then 
      TriggerEvent('desirerp-casino:removeChipsCL', currentBet)
    else
    ...
    TriggerServerEvent(
      "desirerp-slotmachines-sv:spinMachine",
    ...
    --local balance = RPC.execute("desirerp-casino:getCurrentChipCount")
    local balance = RPC.execute('desirerp-casino:getChips')
    --local values = {}
    --table.insert(values, "Balance: $" .. balance)
...
RegisterInterfaceCallback("desirerp-interface-slotmachines:leaveMachine", function(data, cb)
...
function spinSlot(tickRate)
  ...
  for i=1,300,1 do
    ...
    elseif i == 180 then
      ...
      TriggerEvent("desirerp-slotmachines-cl:stopReels", 1, ang1)
    end
    if i < 240 then
    elseif i == 240 then
      ang2 = tickRate.b*22.5-180+0.0
      print(ang2)
      TriggerEvent("desirerp-slotmachines-cl:stopReels", 2, ang2)
    end
    if i < 300 then
    elseif i == 300 then
      ang3 = tickRate.c*22.5-180+0.0
      TriggerEvent("desirerp-slotmachines-cl:stopReels", 3, ang3)
    end				
    Citizen.Wait(10)
  end
  TriggerServerEvent('desirerp-slotmachines-sv:slotsCheckWin', tickRate, data)
end

RegisterNetEvent('desirerp-slotmachines-cl:startSpin')
AddEventHandler('desirerp-slotmachines-cl:startSpin',function(tickRate)
  spinSlot(tickRate)
end)
...
Citizen.CreateThread(function()
  exports["desirerp-polyzone"]:AddBoxZone("casino_door_entry", vector3(959.33, 33.32, 71.91), 2, 5, {
    heading=330,
    --debugPoly=true,
    minZ=69.91,
    maxZ=73.91

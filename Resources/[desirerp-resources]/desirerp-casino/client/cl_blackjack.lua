                ...
                if not timeoutHowToBlackjack then
                    if blackjackTableData[closestChair] == false then
                        if exports['desirerp-inventory']:hasEnoughOfItem('casinomember', 1) then
                            exports["desirerp-interface"]:showInteraction("Press [E] to play blackjack", 'info')
                        else
                            exports["desirerp-interface"]:showInteraction("Press [E] to play blackjack", 'error')
                        end
                    else 
                        exports["desirerp-interface"]:showInteraction("This seat is taken")
                        Citizen.Wait(3000)
...
AddEventHandler("Blackjack:successBlackjackBet",function()
    ...
    TriggerEvent("desirerp-blackjack:close")
...
Citizen.CreateThread(function()
    while true do
        if drawTimerBar then
            if timeLeft == 15 then 
                exports['desirerp-interface']:showInteraction('Time: '..timeLeft)
                Citizen.Wait(2500)
                exports['desirerp-interface']:hideInteraction()
...
AddEventHandler("Blackjack:beginBetsBlackjack",function(gameID,tableId)
    ...
    Citizen.CreateThread(function()
        ...
        if not bettedThisRound then
            ...
            TriggerEvent("desirerp-context:closemenu")
        end
    end)
end)
...
AddEventHandler("betmenu", function()
    exports['desirerp-interface']:openApplication('textbox', {
        callbackUrl = 'desirerp-blackjack:betamount',
        key = 1,
        items = {
          {
            icon = "money-bill",
...
RegisterInterfaceCallback('desirerp-blackjack:betamount', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    local chips = RPC.execute('desirerp-casino:getChips')
	...
	local menuCasino = {
		{
            ...
            key = "BJ.Hit",
			action = 'desirerp-blackjack:hit',
        },
		{
            title = "Stand",
            description = "Pussio Fam !",
            key = "BJ.Stand",
			action = 'desirerp-blackjack:stand',
        },
    }
    exports["desirerp-interface"]:showContextMenu(menuCasino)
end)

RegisterInterfaceCallback('desirerp-blackjack:stand', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('standcasino')
end)

RegisterInterfaceCallback('desirerp-blackjack:hit', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterNetEvent("desirerp-blackjack:close", function()
    local pValues = {
        "line 1",
        "line 2",
    }

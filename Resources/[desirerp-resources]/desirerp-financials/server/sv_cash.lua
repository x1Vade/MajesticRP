...
RegisterNetEvent("desirerp-financials:cash:get")
AddEventHandler("desirerp-financials:cash:get", function(pSource)
    TriggerClientEvent("desirerp-financials:cash", pSource, getCurrentCash(pSource))
end)

RegisterNetEvent("desirerp-financials:cash:give")
AddEventHandler("desirerp-financials:cash:give", function(pSource,pTargetId, pAmount)
    if pAmount <= 0 then return end

    local user = exports["desirerp-base"]:getModule("Player"):GetUser(pSource)
    if not user then return end

    local recipient = exports["desirerp-base"]:getModule("Player"):GetUser(pTargetId)
    ...
    if getCurrentCash(pSource) - pAmount > 0 then
        ...
        TriggerClientEvent("desirerp-financials:cash:give", pSource, getCurrentCash(pSource))
    else
...
RPC.register("desirerp-financials:getCurrentCash", function(pSource)
...
function doAddCashToUser(pSource, pAmount)
    ...
    if _lastPay then
        ...
        if _lastPay.acount >= 4 then
            exports["desirerp-log"]:AddLog("Exploiter", user, "User attempted to give themself money continually in less than 4 seconds", {money_to_give = total})

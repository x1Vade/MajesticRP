RegisterNetEvent('desirerp-heists:bay_city:first_door')
AddEventHandler('desirerp-heists:bay_city:first_door', function()
    ...
    if pd >= 4 then
        if distance < 1.5 then
            if exports["desirerp-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                firstDoorBC()
                TriggerServerEvent('desirerp-heists:bayCityLog')
                TriggerEvent('desirerp-dispatch:bay-city-bank')
                ...
                exports['desirerp-thermite']:OpenThermiteGame(function(success)
                    if success then
                        ...
                        TriggerServerEvent('desirerp-doors:change-lock-state', 563, false) 
                        Citizen.Wait(3000)
...
RegisterNetEvent('desirerp-heists:bay_city:second_door')
AddEventHandler('desirerp-heists:bay_city:second_door', function()
    local pd = RPC.execute("desirerp-police:getActiveUnits")
    if pd >= 4 then
        TriggerEvent('desirerp-dispatch:bay-city-bank')
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['desirerp-taskbar']:taskBar(15000, 'Opening Door.')
        if finished == 100 then
            TriggerEvent('aspect:createTrolleys')
            TriggerServerEvent('pStopSecondDoor')
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerServerEvent('desirerp-doors:change-lock-state', 564, false) 
            -- Create Trolleys.
        end
    end
end)

RegisterNetEvent('desirerp-heists:bay_city:second_door:sv')
AddEventHandler('desirerp-heists:bay_city:second_door:sv', function()
    TriggerServerEvent('desirerp-heists:second_door:cl')
end)

exports["desirerp-polytarget"]:AddCircleZone("desirerp_baycity_unlock", vector3(-1309.13, -815.79, 17.7), 0.45, {
    useZ = true
})

exports["desirerp-interact"]:AddPeekEntryByPolyTarget("desirerp_baycity_unlock", {{
    event = "desirerp-heists:bay_city:second_door:sv",
...
AddEventHandler("desirerp-inventory:itemUsed", function(item)
    if item == "thermitecharge" then
        TriggerServerEvent('desirerp-heists:first_door:cl')
    end
end)
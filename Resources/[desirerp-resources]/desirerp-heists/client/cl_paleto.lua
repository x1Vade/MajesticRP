...
RegisterNetEvent('desirerp-paleto:box')
AddEventHandler('desirerp-paleto:box', function()
    ...
    if distance < 1.5 then
        if exports["desirerp-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
            ...
            PaletoStep1()
            TriggerEvent('desirerp-dispatch:paletobank')
...
RegisterNetEvent('desirerp-paleto:startpaleto')
AddEventHandler('desirerp-paleto:startpaleto', function()
    local player = GetEntityCoords(PlayerPedId())
    local distance = #(vector3(-102.1473, 6463.0869, 31.634153) - player)
    if distance < 1.5 then
        local Police = RPC.execute("desirerp-police:getActiveUnits")
        if Police >= 4 then
            if exports["desirerp-inventory"]:hasEnoughOfItem("heistlaptop2", 1) then
                StartPaleto()
                TriggerEvent('desirerp-dispatch:paletobank')
                TriggerEvent("client:newStress",true, 10)
...
function OnHackSuccess()
    ...
    TriggerEvent("inventory:removeItem", "heistlaptop2", 1)
    TriggerServerEvent('desirerp-paleto:startCoolDown')
    loadDicts()
    ...
    TriggerServerEvent('desirerp-doors:change-lock-state', 45, false) 
    CreateMoney()
end

RegisterNetEvent('desirerp-paletogold')
AddEventHandler('desirerp-paletogold', function()
    print('called 2?')
    TriggerServerEvent('desirerp-paleto:tut_tut')
end)

AddEventHandler("desirerp-inventory:itemUsed", function(pItem)
    if pItem == "thermitecharge" then
        TriggerEvent('desirerp-paleto:box')
    elseif pItem == "heistlaptop2" then
        TriggerServerEvent("desirerp-paleto:hacklaptop")
    end
end)
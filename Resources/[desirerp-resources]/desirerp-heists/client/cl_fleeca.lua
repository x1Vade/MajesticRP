...
RegisterInterfaceCallback('desirerp-fleeca:email_1', function(data, cb)
...
RegisterInterfaceCallback('desirerp-fleeca:email_2', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback('desirerp-fleeca:email_3', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback('desirerp-fleeca:email_4', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback('desirerp-fleeca:email_5', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback('desirerp-fleeca:email_6', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterNetEvent('desirerp-robbery:usb')
AddEventHandler('desirerp-robbery:usb', function()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
            ...
            if closestBank ~= nil then
                ...
                if dist < 1.5 then				
                    if not Config.SmallBanks[closestBank]["isOpened"] then 
                        if Police >= 4 then
                            TriggerServerEvent('desirerp-heists:fleecaBankLog')
                            TriggerServerEvent("desirerp-heists:availabilty")
...
RegisterNetEvent('desirerp-robbery:client:setBankState')
AddEventHandler('desirerp-robbery:client:setBankState', function(bankId, state)
...
RegisterNetEvent('desirerp-robbery:client:enableAllBankSecurity')
AddEventHandler('desirerp-robbery:client:enableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = true
    end
end)

RegisterNetEvent('desirerp-robbery:client:disableAllBankSecurity')
AddEventHandler('desirerp-robbery:client:disableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = false
    end
end)

RegisterNetEvent('desirerp-robbery:client:BankSecurity')
AddEventHandler('desirerp-robbery:client:BankSecurity', function(key, status)
...
RegisterNetEvent('desirerp-robbery:client:setLockerState')
AddEventHandler('desirerp-robbery:client:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "pacific" then	
        print('[QUEER]')
    else
...
RegisterNetEvent('desirerp-robbery:client:ResetFleecaLockers')
AddEventHandler('desirerp-robbery:client:ResetFleecaLockers', function(BankId)
...
AddEventHandler("fleeca:loot", function()
    local card = exports["desirerp-taskbar"]:taskBar(40000,"Taking cash...")
    if card == 100 then
...
RegisterNetEvent("desirerp-fleeca:grab")
AddEventHandler("desirerp-fleeca:grab", function()

...
if store_safe_1 == false then
    ...
    local ply = PlayerId()
    if police >= 2 then
        if exports["desirerp-inventory"]:hasEnoughOfItem("safecrackingkit", 1) then
            if store_crack_1 == true then
                TriggerEvent("desirerp-dispatch:storerobbery2")
                exports['desirerp-thermite']:OpenThermiteGame(function(success)
                    if success then
                        TriggerEvent('desirerp-dispatch:storerobbery')
...
RegisterNetEvent("desirerp-heists:safe:rob", function()
    local policecount = RPC.execute("desirerp-police:getActiveUnits")
    ...
    if isRobbed then
     ...
     if policecount >= 2 then
        if GetEntityHealth(obj) < 800 then
            local finished = exports["desirerp-ui"]:taskBarSkill(25000, math.random(10, 20))
            if (finished == 100) then
                TriggerEvent("desirerp-dispatch:storerobbery2")
                local finished = exports["desirerp-ui"]:taskBarSkill(30000, math.random(10, 20))
                if (finished == 100) then
                    TriggerEvent("desirerp-dispatch:storerobbery2")
                    ClearPedTasks(PlayerPedId())
